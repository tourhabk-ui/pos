...
> Снято 2026-09-26 с настоящего PostgreSQL 16.13 (Ubuntu 16.13-0ubuntu0.24.04.1): baseline прода (`lib/database/baseline/schema-baseline.sql`, снимок 2026-08-15) + миграции новее него, накатанные штатным раннером. Последняя миграция в снимке: `1032_volcano_colloquial_aliases.sql`.
...
erDiagram
  ...
  users ||--o{ partners : "agent_rate_set_by"
...
**commission_payouts** · 16 кол. · PK id · agent_id → users.id, paid_by → users.id, rejected_by → users.id · на неё ссылаются: agent_payout_items · индексов 4

`id uuid!=` `agent_id uuid!` `total_amount numeric!` `status varchar!=` `payment_method varchar` `payout_date timestamp` `notes text` `created_at timestamp=` `updated_at timestamp=` `from_sales boolean!=` `paid_by uuid` `paid_at timestamptz` `paid_reason text` `rejected_by uuid` `rejected_at timestamptz` `reject_reason text`
...
**mchs_registrations** · 14 кол. · PK id · booking_id → bookings.id, operator_booking_id → operator_bookings.id, operator_id → partners.id · индексов 6

`id uuid!=` `booking_id uuid` `operator_id uuid` `group_composition jsonb!` `route text!` `start_date date!` `end_date date!` `guide_contacts jsonb!` `emergency_contacts jsonb!` `status varchar=` `mchs_reference text` `created_at timestamptz=` `updated_at timestamptz=` `operator_booking_id bigint`
...
**place_description_drafts** · 10 кол. · PK place_id, source · place_id → places.id, reviewed_by → users.id · индексов 2
...
**place_volcano_links** · 4 кол. · PK place_id, volcano_place_id · индексов 2

`place_id text!` `volcano_place_id text!` `reason text!` `created_at timestamptz!=`
...
**tours** · 26 кол. · PK id · guide_id → partners.id, operator_id → partners.id, route_id → kamchatka_routes.id · на неё ссылаются: bookings, conversations, guide_earnings, reviews, tour_assets, tour_departures · индексов 11 · триггеры: update_tours_updated_at
...
**guide_certifications** · 15 кол. · PK id · guide_id → partners.id, reviewed_by → users.id · индексов 2 · триггеры: update_guide_certifications_updated_at

`id uuid!=` `guide_id uuid!` `name text!` `issuing_authority text!` `issue_date date` `expiry_date date` `certificate_number text` `document_url text` `is_verified boolean=` `created_at timestamptz=` `updated_at timestamptz=` `source text` `reviewed_at timestamptz` `reviewed_by uuid` `review_comment text`
...
**guide_operator_invites** · 7 кол. · PK id · guide_partner_id → partners.id, invited_by → users.id, operator_id → partners.id · индексов 4

`id uuid!=` `operator_id uuid!` `guide_partner_id uuid!` `status text!=` `invited_by uuid` `created_at timestamptz!=` `responded_at timestamptz`
...
**guide_schedule** · 20 кол. · PK id · guide_id → partners.id, operator_booking_id → operator_bookings.id · на неё ссылаются: guide_earnings, guide_groups · индексов 7 · триггеры: update_guide_schedule_updated_at

`id uuid!=` `guide_id uuid` `tour_date date!` `start_time time without time zone!` `end_time time without time zone` `meeting_point varchar` `participants_count integer=` `max_participants integer` `status varchar=` `weather_conditions jsonb` `safety_notes text` `special_requirements text` `created_at timestamptz=` `updated_at timestamptz=` `title text` `description text` `location_name text` `location jsonb` `notes text` `operator_booking_id bigint`
...
**agent_payout_items** · 9 кол. · PK id · agent_user_id → users.id, booking_id → operator_bookings.id, payout_id → commission_payouts.id · индексов 4

`id uuid!=` `payout_id uuid!` `agent_user_id uuid!` `booking_id bigint!` `sale_amount numeric!` `rate numeric!` `amount numeric!` `released_at timestamptz` `created_at timestamptz!=`
...
**operator_client_notes** · 5 кол. · PK operator_id, user_id · operator_id → partners.id, user_id → users.id · индексов 1

`operator_id uuid!` `user_id uuid!` `tags text[]!=` `telegram text` `updated_at timestamptz!=`
...
`id bigint!=` `observed_date date!` `volcano_name text!` `volcano_name_en text` `place_ark_id uuid` `color text` `color_raw text!` `color_unknown_reason text` `seismicity text` `fetched_at timestamptz!=`
