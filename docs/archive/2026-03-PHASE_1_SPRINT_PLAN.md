...
| `robots.txt` исправлен (домен vedarai.ru) | ✅ | 0a9af271 |
...
Проверить build ID:
```bash
curl -s https://vedarai.ru/ | grep -o '[a-zA-Z0-9_-]\{20,\}' | head -3
```
...
```bash
curl -X POST https://vedarai.ru/api/admin/migrations/apply \
...
```bash
curl -X POST https://vedarai.ru/api/leads \
  -H "Content-Type: application/json" \
...
curl -H "Authorization: Bearer <ADMIN_JWT>" \
  https://vedarai.ru/api/leads/<LEAD_ID>/proposal/pdf \
  --output test.pdf
```
...
Регистрация: vedarai.ru/auth/register?role=operator
```
...
| Метрика | Цель | Факт |
|---------|------|------|
| OperatorPromo видна на vedarai.ru | ДА | ❌ (deploy pending) |
