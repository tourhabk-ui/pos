...
// bumped v30: разовая уборка того, что накопилось с прошлой (02.09). Смена
// имени — единственное, что чистило кэш целиком, и делается она рукой: 22 раза
// за всю историю. Деплой идёт на каждый пуш в main, с 02.09 их прошло 1301 —
// столько наборов чанков успело осесть. Впредь копиться не даст эвикция статики
// (evictOldStaticAssets ниже), а этот бамп разбирает уже накопленное.
const CACHE_NAME = 'kamchatour-v30';
...
const OPTIONAL_URLS = [
  ...
  '/offline',
  '/offline/manage',
  // `/planning` переехал в FIELD_URLS выше (13.09): он стал стартовым экраном
  // приложения, а здесь лежал без повторов — сеть моргнула при установке, и
  // вход в приложение офлайн не поднимался вовсе.
  '/ai-assistant',
];
...
const FIELD_URLS = [
  '/field-check',
  // Стартовый экран установленного приложения (`start_url` в manifest.json,
  // 13.09). До этого дня приложение открывалось главной витриной, хотя ставят
  // его ради маршрута; теперь система при запуске просит ИМЕННО этот адрес —
  // значит промах прекэша здесь стоит не карточки, а всего входа: человек в
  // поле нажимает иконку и получает «нет соединения» вместо кокпита.
  //
  // Оба вида адреса нужны РАЗНЫМ входам, и подменить один другим нельзя:
  // `caches.match` без `ignoreSearch` считает `?mode=trail` другим ресурсом,
  // поэтому запуск с иконки не нашёл бы голый `/planning`, а переход из
  // таб-бара — адрес с параметром. Дубля данных тут нет: режим экрана
  // читается на клиенте, сервер отдаёт обоим один и тот же HTML.
  '/planning',
  '/planning?mode=trail',
];
...
/**
 * LRU-эвикция СТАТИКИ — единственная ветка кэша, которая росла без потолка.
 *
 * У карточек мест, туров и планов потолок есть с самого начала. У
 * `/_next/static/**` его не было вовсе, и вот почему это копится: имя чанка
 * содержит хеш сборки, после каждого деплоя страница тянет НОВЫЕ файлы, а
 * старые остаются в кэше навсегда. Единственная уборка — смена `CACHE_NAME`,
 * а она делается рукой: 22 раза за всю историю, последняя 02.09. Деплой идёт
 * на каждый пуш в `main` — с той уборки их прошло 1301 (замер 13.09).
 *
 * Потолок взят ЗАМЕРОМ, а не на глаз: `npm run build` на 13.09 даёт 1179
 * файлов `.js`/`.css` — это ВСЯ сборка целиком, включая страницы, которые
 * полевой телефон не открывает никогда. Значит кэш, где файлов больше, чем в
 * полной сборке, заведомо держит чанки прошлых деплоев. Полторы тысячи —
 * тот же замер с запасом на рост репозитория.
 *
 * Что удаляется первым: `cache.keys()` отдаёт записи в порядке ВСТАВКИ, а
 * вставлялись они по мере запроса — то есть первыми уходят чанки самых старых
 * сборок. Ровно те, что уже никому не нужны.
 *
 * Чего эвикция НЕ делает: не трогает критичный прекэш (`/safety/*.js`,
 * Leaflet, иконка — они не под `/_next/static/`), офлайн-страницы и тайлы.
 * СОС от уборки не зависит.
 */
const MAX_STATIC_ASSETS = 1500;

async function evictOldStaticAssets(cache) {
  const keys = await cache.keys();
  const staticKeys = keys.filter((req) => req.url.includes('/_next/static/'));
  if (staticKeys.length > MAX_STATIC_ASSETS) {
    const toDelete = staticKeys.slice(0, staticKeys.length - MAX_STATIC_ASSETS);
        ...
        return fetch(request).then((response) => {
          if (response.ok) {
            ...
            caches.open(CACHE_NAME).then(async (cache) => {
              ...
              await evictOldStaticAssets(cache);
            });
          }
          return response;
        });
