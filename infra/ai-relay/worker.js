 ...
 *
 * ВНИМАНИЕ (23.08): С OPENROUTER ЭТОТ ЗАМЫСЕЛ НЕ РАБОТАЕТ.
 *
 ...
 *
 * С ANTHROPIC — РАБОТАЕТ (замер с прода 03.09, `anthropic-path-probe` run 2).
 * Запись 23.08 «и Anthropic» была ВЫВОДОМ из случая OpenRouter, а не замером:
 * с прода через этот воркер (`/anthropic`) api.anthropic.com ответил своим
 * телом (HTTP 400 «credit balance is too low» — беда ключа, не пути), прямой
 * адрес с прода — 403 «Request not allowed». Тем же прогоном Cloudflare AI
 * Gateway (`gateway.ai.cloudflare.com/v1/<account>/vedar-ai/anthropic`) тоже
 * довёл до Anthropic. Значит для Claude годятся ОБА пути, и переменная
 * `ANTHROPIC_BASE_URL` на этот воркер стоит на проде не зря. Единственный
 * блокер на 03.09 — нулевой баланс ключа Anthropic.
 *
 * Для OpenRouter по-прежнему нужен хоп ВНЕ сети Cloudflare — см.
 * `infra/ai-relay-vps/`. gh-api/gh-raw с прода работают.
 *
