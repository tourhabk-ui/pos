...
export type NavApp = 'organic' | 'yandex' | 'dgis';
 ...
 *
 * Порядок не случаен: Organic Maps первым, потому что он один из трёх
 * работает без сети — а на Камчатке связь кончается раньше дороги.
...
export function navLinks(
  ...
  const apps: NavApp[] = ['organic', 'yandex', 'dgis'];
  const out: NavLink[] = [];
