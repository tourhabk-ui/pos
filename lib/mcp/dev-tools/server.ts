 ...
 * `.mcp.json` как на то, что задаёт рабочий каталог. Тогда такого файла в
 * репозитории не было вовсе, и докстрока обещала путь, которого нет, — дефект
 * кода (правило 10.09): читающий верил, что достаточно положить файл на место.
 *
 * С 19.09 `.mcp.json` в корне ЕСТЬ (решение владельца), но прежнюю строку он
 * не воскрешает, и это важно не перепутать. Он объявляет, какие MCP-серверы
 * потребляет наш агент — сегодня один, хостовый Inspo, — и ни рабочего
 * каталога, ни запуска ЭТОГО сервера не задаёт: `mcpServers.inspo` не
 * упоминает ни `command`, ни `cwd`. Отсюда сервер по-прежнему не запускается
 * ничем, и cwd обязан задать запускающий.
 */
...
// `.mcp.json`. С 19.09 такой файл есть, но корень он НЕ задаёт: это конфиг
// ПОТРЕБЛЯЕМЫХ серверов (хостовый Inspo по http), в нём нет ни `command`, ни
// `cwd`, и этот сервер он не запускает. Условие по-прежнему никем не
// обеспечивается — запускающий обязан задать cwd сам.
