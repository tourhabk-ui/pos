...
const MCHS_DISTRICT_ZONES: Array<[RegExp, string[]]> = [
  ...
  // (tests/unit/hazard-grade.test.ts). Начилова, Амчигача и Большая Быстрая
  // НЕ внесены: из контейнера разработки Overpass закрыт, а зона по памяти —
  // догадка в системе безопасности; к тому же «Быстрых» на Камчатке две
  // (у Эссо и у Малок). Невнесённая река не ломает ничего: пункт остаётся
  // в ленте без зоны, как и до этой строки.
  [/воровск/i,      ['western']],
];
