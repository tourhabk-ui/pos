...
export function addressesTouristsWithBan(text: string): boolean {
  const lower = text.toLowerCase();
  const audience = /тургрупп|турист|охотник|маломерн|рыбак|восходител/.test(lower);
  const ban = /воздержат|не выходить|не выезжать|не выход[аи]|не совершать|не планировать|отказаться от|не подниматься|не приближ|запрещ|не выпускать|не выходите/.test(lower);
  return audience && ban;
}
