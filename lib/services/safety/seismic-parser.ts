...
export function isServiceStatistics(text: string): boolean {
  ...
  const past = /привлекал[аиось]+|совершен[оы]|зарегистрирован[оы]?|ликвидирован[оы]?|потушен[оы]?|произошл[ои]|выполнен[оы]|спасен[оы]|обследован[оы]/.test(lower);
  if (!past) return false;

  const counted = new RegExp(
    `(?:${COUNT_WORD})\\s*раз|раз[а]?\\s*(?:${COUNT_WORD})|(?:${COUNT_WORD})\\s*(?:пожар|происшеств|выезд|дтп|человек|очаг)`,
    'i',
  ).test(lower) || /\bдважды\b|\bтрижды\b/.test(lower);
