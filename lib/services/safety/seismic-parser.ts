...
export interface SeismicEvent {
  ...
  source_url: string;
  published_at: Date;
  alert_type: 'volcanic_eruption' | 'earthquake' | 'seismic_bulletin' | 'ash_cloud' | 'info' | 'tsunami_warning' | 'flood' | 'fire_danger' | 'road_closure' | 'weather';
  severity: 0 | 1 | 2 | 3;
  title: string;
  description: string;
 ...
 * заслуживает.
 */
const BAN_AUDIENCE = /тургрупп|турист|охотник|маломерн|рыбак|восходител/;
...
const BAN_VERB = /воздерж|не рекоменду|не выходить|не выезжать|не выход[аи]|не совершать|не планировать|отказаться от|не подниматься|не приближ|запрещ|не выпускать|не выходите/;
...
export function isRescueOperationReport(text: string): boolean {
  const lower = text.toLowerCase();
  const operation = /(окаж[еу]т|оказал[аи]?|оказана|оказыва[ею]т)\s+помощь|эвакуирова|деблокирова|(вед[уёе]тся|ведут|ведётся|проводят|организован[аы]?)\s+поиск|подняли на борт|транспортирован|сняли с маршрута/.test(lower);
...
export function classifyMchsItem(
  ...
  if (isDailyBulletin(text) || isRescueOperationReport(text)) {
    return null;
  }
  ...
  } else if (/павод(ок|к)|половодь|подтоплен|уровень воды|реках.*ожидается/i.test(text)) {
