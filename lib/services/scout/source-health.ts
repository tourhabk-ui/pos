...
export const SCOUT_SOURCE_EXPECTATIONS: readonly SourceExpectation[] = [
  ...
  // tg_kammeteo убран 08.09: канал оказался о погоде в Санкт-Петербурге
  // (см. разбор в scout-digest). Порог тишины без источника не нужен.
  ...
  // ── Край и надзор (09.09) ────────────────────────────────────────────────
  //
  // Пороги разные, потому что ритм разный, и ставить их «на глаз одинаково»
  // значило бы получить либо ложную тревогу, либо молчащий сторож.
  //
  // Краевые СМИ пишут ежедневно: у обоих на момент внесения свежайший
  // материал в день замера, у канала 330 постов в превью. Двое суток тишины
  // у ежедневного издания — уже событие, а не выходные.
  //
  // Надзорные ведомства публикуют по поводу, а не по расписанию: у них
  // неделя молчания законна, и требовать от них ежедневности значило бы
  // приучить владельца пролистывать тревоги.
  { key: 'poluostrov_kamchatka', label: 'Полуостров Камчатка', maxSilenceHours: 72 },
  { key: 'tg_kamchatinfo',       label: 'Камчатка-Информ',     maxSilenceHours: 72 },
