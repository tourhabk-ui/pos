...
/**
 * Отказ приёма, названный человеку словами.
 *
 * ── Случай 31.08 ──────────────────────────────────────────────────────────
 *
 * Владелец записал трек до Раздолья и нажал «Остановить». На сервере ноль.
 * Причина не в отправке: рекордер не принял НИ ОДНОЙ засечки, и черновика
 * не появилось вовсе — сохранять было нечего.
 *
 * Улика лежала в его же полевых проверках, ушедших в тот же час с того же
 * телефона: точность 500, 1000 и 100 метров. Порог приёма — 50. Всё, что
 * давал прибор, летело в отброшенные.
 *
 * И экран при этом выглядел как идущая запись. `silent` не загорался: он
 * означает «засечек нет вовсе», а они приходили — просто ни одна не
 * годилась. Счётчики отброшенных считались исправно и не показывались
 * никому. Это ровно §4.0: работа идёт, результата нет, слова нет.
 *
 * Отказ приёма — не «пока ничего», а состояние, которое надо назвать. Порог
 * тут не смягчается: 50 метров выбраны потому, что этой линией мы ЗАМЕНЯЕМ
 * геометрию платформы (см. шапку). Лечится не понижением планки, а тем, что
 * человек узнаёт причину сразу, а не через сутки по пустой таблице.
 */
export const DROP_WORDS: Record<DropReason, string> = {
  accuracy: `Прибор знает место хуже ${ACCEPT_ACCURACY_M} м — точки не пишутся. `
    + 'Выйдите на открытое место или подождите, пока поймаются спутники',
  unknown_accuracy: 'Прибор не говорит, насколько уверен в месте — точки не пишутся',
  jitter: 'Движения не видно — точки не пишутся',
  jump: 'Координата прыгает — прибор теряет место, точки не пишутся',
  bad_number: 'Прибор отдаёт негодные числа — точки не пишутся',
};
