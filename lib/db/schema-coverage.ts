...
export function usedTables(root = process.cwd()): Map<string, Set<string>> {
  ...
  for (const f of listed) {
    ...
    // `EXTRACT(EPOCH FROM col)` — та же беда, что у DISTINCT FROM ниже: слово
    // FROM здесь часть функции, а не начало источника строк. Без отсечения
    // сторож объявил «таблицей» алиас из выражения (14.09: `EXTRACT(EPOCH
    // FROM ai2.created_at)` дало «таблицу ai2»), то есть поднял тревогу на
    // безупречном SQL.
    //
    // Вырезается ровно шапка вызова, а не весь EXTRACT со скобками: разбирать
    // вложенные скобки регуляркой — способ сломаться на первом же сложном
    // выражении, а для нашей цели достаточно убрать само слово FROM.
    txt = txt.replace(/\bEXTRACT\s*\(\s*[a-z_]+\s+FROM\b/gi, 'EXTRACT(');
