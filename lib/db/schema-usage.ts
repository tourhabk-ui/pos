 ...
 * total_price, ...)` падал при каждом вызове, потому что таких колонок у
 * `bookings` нет. (Раньше здесь стояло «bookings оказалась VIEW» — неверно:
 * аудит боевой схемы показал обычную базовую таблицу, просто без `end_date` и
 * `currency`. Вывод от этого не менялся, но причина названа была не та.)
 * Бронирование тура существовало в виде маршрута и не существовало как
 * работающая функция; фронт его не звал, и поломка молчала. Тот же приём,
