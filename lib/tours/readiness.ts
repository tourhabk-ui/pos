...
export function missingFields(r: ReadinessRow): string[] {
  ...
  if (r.pickup_type === null) {
    missing.push('pickup');
  } else if (r.pickup_type === 'meeting_point' && r.pickup_details_chars === 0) {
    // «Встречаемся» без адреса покупателю бесполезно — он не знает, куда
    // прийти. «Заберут» подробностей НЕ требует (владелец 05.09, ответ на
    // вопрос о подвозе: «забирает он туристов сам» — это и есть вся фраза;
    // границы забора оператор уточняет при брони, а требовать их здесь
    // значило бы держать восемь туров вне витрин ради поля, которого
    // владелец не просил). «Добирается сам» — куда ехать, отвечают
    // координаты тура, они проверяются отдельной строкой.
...
export const BLOCKER_LABELS: Record<string, string> = {
  ...
  pickup_details: 'Не указано, где и когда встречаете группу',
