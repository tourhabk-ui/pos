...
/**
 * Места платформы (05.09, форма — 07.09, цвет заливки по категории — 10.09).
 * Точка — факт географии (§9 CLAUDE.md), и на карте она отвечает на два
 * вопроса разом: «что здесь» и «чем опасно». Форма и заливка несут первый
 * ответ, кромка — второй:
 *
 *   - ФОРМА И ЗАЛИВКА — по типу места (`kind`, тот же столбец, что и
 *     фильтр-чипсы): вулкан — силуэт горы своего оранжевого, источник —
 *     кружок с паром своего красного, гейзер — фонтаном своего цвета, и так
 *     у каждого типа (lib/map/place-marker-icons.ts, PLACE_KIND_COLOR — тот
 *     же набор форм и цветов, что у старой Leaflet-карты divIcon-маркерами,
 *     единый источник на обе карты). Заливку выбирает VedarMap
 *     (`styleimagemissing`), не этот файл — здесь только имя иконки;
 *   - КРОМКА — из ДАННЫХ профиля безопасности: место с записанными
 *     опасностями (`hazard_types` не пуст) — кромка цвета тревоги, тот же,
 *     что у обрыва; иначе — кромка фона карты (контраст с любой ступенью
 *     гипсометрии). До 10.09 опасность красила ЗАЛИВКУ (два цвета на все
 *     ~20 типов), и вулкан с записанным профилем терял свой «вулканический»
 *     оранжевый — становился неотличим от источника. Опасность красится по
 *     данным, не по типу: вулкан без профиля не тревожный «на всякий
 *     случай» — это выдумка (§4.0).
 *
 * Подпись (текст рядом с иконкой, ниже) красится отдельно — по опасности,
 * не по категории: список из 383 имён на карте читается быстрее, если
 * тревожный текст всегда одного цвета, а не пятнадцати.
 *
 * До правки 07.09 была только кромка: все места — один кружок двух цветов.
 * Владелец 07.09, после переезда `/map` на VedarMap: «геоточки были все со
 * своими маркерами» — разница типов читалась с одного взгляда на старой
 * карте и терялась на новой не по решению, а по недосмотру миграции.
 *
