...
export function buildVedarStyle(
  ...
  return {
    ...
    // Шрифты и спрайты не подключены намеренно: подписи горизонталей идут
    // системным стеком MapLibre, а внешний CDN глифов сделал бы офлайн
    // ложным — карта «сохранена», а числа на ней не появились.
    glyphs: undefined,
    sources: {
      terrain: {
        type: 'raster-dem',
