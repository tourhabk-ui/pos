...
/**
 * Версия слоя мест — уходит в адрес файла как `?v=`.
 *
 * Равна `run` маркера `.github/triggers/map-places-build.json` последней
 * заливки, и скрипт заливки ОТКАЗЫВАЕТСЯ лить, если они разошлись
 * (scripts/map-tiles/build-places.ts, MAP_PLACES_RUN). Сторож:
 * tests/unit/places-layer-version.test.ts.
 *
 * ── Зачем (17.09) ─────────────────────────────────────────────────────────
 *
 * Владелец трижды за день видел на полевой карте «Озеро Овальное» — запись,
 * скрытую миграцией 947 неделю назад. К вечеру было доказано с раннера, что
 * её нет НИГДЕ на сервере: прод отвечает 404, экспорт по имени — 0 (проба
 * 512), все 123 пакета прочитаны обратно байт в байт (прогон 11). Телефон
 * показывал копию, которой не существует.
 *
 * Ключ файла постоянный (`<region>.places.geojson`), файл под ним
 * переписывается; заголовок `no-cache` обязывает браузер сверяться, но
 * снимок 18:59 — через четыре часа после заливки — показал, что на этом
 * нельзя строить: где именно застрял старый файл (кэш браузера, память
 * открытого PWA, прослойка перед хранилищем), с раннера не видно. Версия в
 ...
 */
export const PLACES_LAYER_VERSION = 13;
...
function placesUrlFor(region: PackRegionId, base: string): string | null {
  return PLACES_BUILT.includes(region)
    ? `${base}/${placesKey(region)}?v=${PLACES_LAYER_VERSION}`
    : null;
}
