...
${route.description ? `Имеющееся описание (бери из него факты, расширь и улучши, ничего не выдумывай сверх него): ${route.description}` : 'Готового описания нет. Используй только название и тип. Пиши обобщённо, без выдуманных чисел и без сочинённых фактов о безопасности.'}

Помоги туристу понять: что это за место и его общий характер, чем интересно, на что обратить внимание при подготовке. Конкретику (высоту, координаты, опасности, сроки) указывай ТОЛЬКО если она есть в переданных данных.`,
    },
  ];
