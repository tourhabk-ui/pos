...
export const MANUAL_ENDPOINTS: Record<string, SchedulerDeclaration> = {
  ...
  'referral-census':           { kind: 'manual', writes: false, note: 'агентские реферальные ссылки в числах: сколько заведено, с какими ставками, сколько броней ими атрибутировано и сколько оплачено. Заведена 20.09 под запуск программы креаторов (#1978): ставку своей комиссии агент задавал САМ телом запроса (до 30%), а кабинет показывал ему «заработано» при отсутствующем пути выплаты агенту. Прежде чем чинить, нужен размер — «наверное, их нет» в этом репозитории дважды оказывалось втрое неверным' },
  ...
  // deepseek-model-probe.yml по маркеру — объявление было бы вторым ответом
  // на тот же вопрос (тот же довод, что у 'tochka-check' и 'ai-channel-check').
  'source-probe':              { kind: 'manual', writes: false, note: 'достижимы ли кандидаты в источники С ПРОДА: раннер стоит вне РФ, и его сетевой отказ по госсайту неотличим от мёртвого хоста. Список адресов зашит в код — роут без параметров, иначе это SSRF' },
  ...
  // вопрос и потому ходят одним прогоном: доходит ли заявка до оператора,
  // пишет ли маяк середины воронки и каковы сами числа.
  ...
  // 'place-photos-commons' объявления здесь не несёт: его зовёт
  // place-photos-commons.yml по маркеру — объявление было бы вторым
  // ответом на тот же вопрос (тот же довод, что у 'tochka-check' выше).
  // Фото мест с Commons по координатам: GET — перепись, POST — сбор
  // партией с сухим прогоном по умолчанию. Боевая запись остаётся
  // решением человека: geosearch находит снимки РЯДОМ с точкой, и рядом
  // с вулканом бывает парковка.
  ...
  // вторым ответом на тот же вопрос (тот же довод, что у 'tochka-check' и
  // 'ai-channel-check'). Переезд в хранилище был объявлен ручным до 18.09,
  // пока звать его действительно было нечем; в тот день владелец решил
  // переехать, у задачи появился актуатор, и объявление стало лишним.
  'place-description-geo':      { kind: 'manual', writes: false, note: 'описание места называет географический объект, а координата места от него далеко. Заведена 20.09: за два дня сочинённое описание нашлось у ЧЕТЫРЁХ мест подряд (Каньон Опасный, Крылья Гамулов, Каньон Сноубордистов, Сопка Никольская) — и все четыре СЛУЧАЙНО, по ходу другой работы. Улика, не приговор: вулкан бывает виден за сто километров. Объекты вне нашей базы невидимы по построению — такие места идут в отдельный счёт nothing_to_check, а не в чистые. С 26.09 там же блок voice — описания, написанные путевой заметкой от первого лица (Авачинский). Только чтение' },
  'place-photo-coverage':      { kind: 'manual', writes: false, note: 'у каких живых мест есть ПОКАЗЫВАЕМОЕ фото, а у каких только скрытое или никакого. Заведена 19.09: владелец трижды за вечер спросил «где фото», и каждый раз ответ приходилось собирать заново — переписи не было, хотя снимков в базе шесть сотен, а показывается меньше пятой части (скрейп, генерации и чужие без автора не показываются, каждое по своей причине). `?name=` отвечает про одно место словами. Только чтение' },
  'images-in-s3':              { kind: 'manual', writes: false, note: 'что из снимков УЖЕ лежит в хранилище: список с именем места, родом и ключом, плюс счётчики «в S3 / ещё байтами / ни там ни там». Заведена 19.09 по просьбе владельца дать список перевезённых — актуатор его дать не может, он печатает только последний ответ партии. Разделить по прогонам нечем: отметки времени переезда в таблице нет. 19.09 добавлена проверка доступности `?check=N`: HEAD по объекту без наших ключей — так же, как за снимком идёт браузер туриста. Заливка проверяет объект ОДИН РАЗ при переезде, а закрыться он может позже, и тогда в базе ссылка, байты обнулены, а на экране пусто. Только чтение' },
  ...
  'weathernext-probe':         { kind: 'manual', writes: false, note: 'где детерминированный прогон Open-Meteo и вероятностный ансамбль WeatherNext 2 расходятся по зонам Камчатки (#1787). Меряется РАСХОЖДЕНИЕ, не точность: точность требует наблюдений, которых у нас нет. Только чтение' },
  'vision-probe':              { kind: 'manual', writes: false, note: 'почему Кузьмич не разбирает фото: все ступени зрения по очереди на вшитом 310-байтном снимке, у каждой исход и причина. Отдельный исход answers_but_blind — ответила текстом, но цвета не назвала, значит снимок до модели не дошёл. Только чтение, наружу ничего не публикуется' },
  'voice-probe':               { kind: 'manual', writes: false, note: 'может ли Кузьмич говорить голосом через ключ DashScope (#1992): модели синтеза из каталога ключа, синтез русской фразы, приговор по скачанным байтам аудио, ссылка на аудио — для проверки на слух. Не больше двух моделей за прогон, только чтение' },
  ...
  'operator-screens-check':    { kind: 'manual', writes: false, note: 'разбираются ли запросы пяти экранов кабинета оператора на схеме ПРОДА: починка #1807 доказана базой из миграций, а схема прода с ней совпадает не везде (schema-coverage). PREPARE без выполнения' },
  'user-photo-hero':           { kind: 'manual', writes: true, note: 'снимки туристов: перепись (кто, к какому месту, в каком состоянии, есть ли у места главное фото) и перенос выбранного в герои карточки. Тот же перенос, что у кнопки на карточке — общая функция lib/places/user-photo-hero. Сухой прогон по умолчанию, партия ≤10, author обязателен: у пути по CRON_SECRET владельца аккаунта нет, и подставить имя загрузившего значило бы опубликовать чужие ПД по догадке' },
