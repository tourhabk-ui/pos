...
export const MANUAL_ENDPOINTS: Record<string, SchedulerDeclaration> = {
  ...
  'images-in-s3':              { kind: 'manual', writes: false, note: 'что из снимков УЖЕ лежит в хранилище: список с именем места, родом и ключом, плюс счётчики «в S3 / ещё байтами / ни там ни там». Заведена 19.09 по просьбе владельца дать список перевезённых — актуатор его дать не может, он печатает только последний ответ партии. Разделить по прогонам нечем: отметки времени переезда в таблице нет. Только чтение' },
