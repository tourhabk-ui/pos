...
5 — все место-специфичные факты либо подтверждены контекстом, либо ответ честно говорит "нет точных данных" вместо выдумки. Это же 5 — ОБЩЕИЗВЕСТНЫЙ протокол первой помощи или безопасности, не привязанный к конкретному месту и не противоречащий контексту: "звони 112 при ЧП", "спускайся при симптомах горной болезни", "кипяти воду не из проверенного источника", "не подходи к дикому животному и не поворачивайся к нему спиной". Это не выдумка, а общее знание, верное независимо от того, о каком месте вопрос — не проверяй его по контексту и не штрафуй за отсутствие там.
3 — в целом опирается на контекст, но есть непроверяемые уточнения без явной выдумки конкретики.
1 — есть МЕСТО-СПЕЦИФИЧНЫЙ факт БЕЗОПАСНОСТИ (статус ЭТОГО места, опасность НА ЭТОМ маршруте, проходимость, сложность, наличие связи ЗДЕСЬ), которого нет в контексте и который похож на выдумку. Общий совет вроде примеров из пункта 5 сюда НИКОГДА не попадает — граница не в тоне ответа, а в том, привязан ли факт к конкретному месту.
