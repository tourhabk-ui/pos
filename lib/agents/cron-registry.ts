...
export const CRON_REGISTRY: CronEntry[] = [
  ...
  {
    // Правило владельца 18.09: снимок места живёт в хранилище, а не в базе.
    // Заведено расписанием, а не абзацем: переезд был НАПИСАН 08.09 и десять
    // дней не запускался — звать его было нечем. Уборщик закрывает класс:
    // что бы в базу ни легло, к следующему прогону оно уедет.
    key: 'images-to-s3', label: 'Снимки мест в хранилище',
    description: 'Перенос снимков мест из байтов в базе в S3: заливка, чтение обратно, сверка размера, и только потом обнуление байтов.',
    workflow: 'cron-images-to-s3.yml', cron: '41 18 * * *', schedule: 'ежедневно · 18:41 UTC',
    everyMin: DAY, tier: 'ops', agentId: 'images-to-s3', triggerable: true,
  },
  // ── Качество ─────────────────────────────────────────────────────────────
  {
...
export const CRON_IDLE_MEANING: Record<string, IdleMeaning> = {
  // ── Операции ────────────────────────────────────────────────────────────
  // Ноль перевезённых снимков — ЦЕЛЬ, а не поломка: значит в базе байтов уже
  // не осталось и правило «фото всегда на S3» соблюдается само. Уборщик
  // затем и заведён, чтобы однажды перестать находить работу. Настоящая его
  // поломка выглядит иначе — не пустой прогон, а ненулевой `failed_count`
  // или HTTP не 200, и оба валят прогон на месте, не дожидаясь серии.
  'images-to-s3': 'normal',
