...
export const RSS_SOURCES: ScoutSource[] = [
  ...
  // ── arXiv: первоисточник работ, а не пересказ о них (19.09) ──────────────
  //
  // Слово владельца: «добавь arxiv.org/abs в один из ресурсов поиска
  // разведчика — там очень интересные есть описания и инструкции». Все
  // восемь AI-источников выше — блоги и ленты: о работе мы узнаём тогда,
  // когда о ней кто-то напишет, и в его пересказе. Сама статья выходит на
  // arXiv раньше и с методом внутри.
  //
  // ── Почему API, а не лента раздела ───────────────────────────────────────
  //
  // Полный cs.AI — больше сотни работ в сутки, а разбор берёт из фида первые
  // ПЯТЬ (parseRssItems). Пять случайных из сотни — это не разведка, а
  // лотерея. Поисковый запрос сужает до того, ради чего источник и внесён:
  // агентные системы. Ответ — Atom с <entry>, и первым <link href> в каждой
  // записи идёт ровно `arxiv.org/abs/...`, то есть тот адрес, который
  // владелец и назвал.
  //
  // ── Чего про этот адрес НЕ проверено ─────────────────────────────────────
  //
  // Остальные источники этого файла вносились ПОСЛЕ переписи (/census с края
  // Cloudflare, source-discovery). Здесь переписи не было: arxiv.org из
  // среды, где правился файл, не достижим вовсе, и выдавать память за замер
  // нельзя. Проверку даёт ПЕРВЫЙ ЖЕ прогон: отчёт здоровья назовёт
  // `status` ('ok' / 'empty' / 'error' с причиной) и `via` ('direct' —
  // читается с прода, 'relay' — только через Cloudflare). Пустая выдача
  // здесь будет значить «адрес или запрос неверны», а не «на arXiv тихо»:
  // по такому запросу работы выходят ежедневно.
  //
  // Второй раздел (cs.CL) вносить только после того, как первый подтвердится
  // живым, — иначе одной ошибкой в адресе заведём сразу два мёртвых источника.
  {
    key: 'arxiv',
    url: 'https://export.arxiv.org/api/query?search_query=cat:cs.AI+AND+abs:agent&sortBy=submittedDate&sortOrder=descending&max_results=10',
    label: 'arXiv cs.AI',
    category: 'ai',
    kind: 'rss',
  },
