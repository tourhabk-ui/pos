    ...
    for (const tour of tours.rows) {
      try {
        const prompt = [
          ...
          '',
          'Перепиши описание тура для платформы KamchatourHub (Камчатка). Тон деловой и заслуживающий доверия, а не навязчиво-рекламный — приоритет платформы безопасность и честность.',
          'СТРОГО: используй только факты из названия и текущего описания. НЕ выдумывай длительность, маршрут, цену, что включено, уровень сложности или иные детали, которых нет во входных данных.',
          'Требования: 2-3 предложения, конкретно, на русском языке. Без эмодзи, без кавычек по краям.',
        ...
        const messages: ChatMessage[] = [
          {
            role: 'system',
            content: 'Ты редактор турплатформы KamchatourHub (Камчатка). Пишешь честные, точные описания туров без выдуманных фактов и без эмодзи. Приоритет — доверие и безопасность туриста, а не максимизация продаж.',
          },
...
async function executePromptOptimize(task: ExecutionTask): Promise<ExecutionResult> {
  ...
  try {
    ...
    const systemPrompt = `Ты AI-архитектор платформы KamchatourHub (TourHab, Камчатка). Оптимизируешь промпты AI-агентов по латентности и качеству.
Опирайся только на задачу и KPI, переданные пользователем. Если конкретных данных о проблеме недостаточно — не выдумывай несуществующие проблемы агентов; вместо этого предложи безопасные общие приёмы снижения латентности (короче промпт, меньше токенов вывода, кэш, более быстрая модель в waterfall) и честно отметь, что нужна телеметрия.
Каждая оптимизация должна быть конкретной и применимой.
Верни ТОЛЬКО JSON-массив из ровно 3 объектов, без markdown:
[{ "agent": "<идентификатор агента или 'all'>", "issue": "<в чём проблема>", "fix": "<конкретное изменение промпта или конфигурации>" }]`;
...
Выдай ровно 3 приоритетных оптимизации в указанном JSON-формате.`,
      },
    ];
...
/**
 * ЭФФЕКТ инициативы + бухгалтерия agent_approvals (in_progress → done /
 * failed / retry). Жизненный цикл ядра (задача, policy, идемпотентность,
 * события) этим кодом НЕ ведётся — его держит вызывающий: kernel worker
 * (lib/agents/kernel/adapters/initiative-tasks.ts), который захватил задачу
 * и сверил payload до вызова. Прямых вызовов мимо worker'а не осталось
 * (модель автономии 27.08: execute-all ставит в очередь, не исполняет).
 */
export async function executeInitiativeEffect(task: ExecutionTask): Promise<ExecutionResult> {
