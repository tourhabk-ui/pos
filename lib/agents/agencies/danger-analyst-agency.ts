  ...
  const messages: ChatMessage[] = [
    {
      role: 'system',
      content:
        'Ты — аналитик угроз для оперативного штаба по безопасности туристов на Камчатке. ' +
        'От точности твоего заключения зависят решения об эвакуации, поэтому достоверность важнее полноты.\n' +
        'ЖЁСТКИЕ ПРАВИЛА ПРОТИВ ВЫДУМЫВАНИЯ:\n' +
        '- Опирайся ИСКЛЮЧИТЕЛЬНО на переданные сейсмические и вулканические события и переданную предварительную оценку риска. ' +
        'Не добавляй угрозы (цунами, лавины, извержения, сели, штормы), если их нет во входных данных.\n' +
        '- Не называй магнитуды, высоты пепла, даты или названия вулканов, которых нет во входных данных. Не округляй и не оценивай числа «на глаз».\n' +
        '- Не повышай и не понижай уровень риска относительно переданной предварительной оценки; объясняй именно её.\n' +
        '- Если данных недостаточно для уверенного вывода — прямо напиши об этом, не домысливай.\n' +
        'Пиши точно, лаконично, на русском. Без заголовков, без воды, без эмодзи.',
    },
    {
      role: 'user',
      content:
        `Составь заключение (3-5 предложений) для оперативного штаба СТРОГО по данным ниже, ничего не добавляя от себя.\n\n` +
        `ЗОНА: ${ZONE_NAMES[data.zone]}\n` +
        `ВУЛКАНЫ ЗОНЫ (справочно, не считать их активными без событий ниже): ${volcanosList}\n` +
        ...
        `ПРЕДВАРИТЕЛЬНАЯ ОЦЕНКА РИСКА (расчётная, опирайся на неё): ${quickScore}/100\n\n` +
        `Укажи только на основе данных выше: характер фактической угрозы, риск для туристов, приоритетность реагирования. ` +
        `Если событий нет или они незначительны — так и напиши.`,
    },
  ];
