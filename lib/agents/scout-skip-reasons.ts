...
export const SKIP_REASON_LABELS: Record<string, string> = {
  // safety-ingest (#1759): прогон был, источники или запись отдали ошибку.
  ...
  // 07.09: в канал ушёл выпуск ИЗ ОДНОГО пункта. Сбор был исправен — 16
  // источников из 16 живы, — но предыдущий прогон за 25 минут до этого уже
  // разобрал материал, и межпрогонный дедуп снял 77 записей из 78. Ворот на
  // «слишком мало» не было: ноль умел молчать, а единица шла в печать.
  too_few_signals: 'свежих сигналов меньше, чем нужно на выпуск — не дайджест, а обрывок',
  ...
  ai_post_too_thin: 'в AI-посте меньше двух полных материалов — день пропущен, чтобы не выпускать обрывок под шапкой дайджеста',
  ...
  // ── Не разведка: eval Кузьмича (08.09) ──────────────────────────────────
  // Словарь давно перестал быть только разведческим (см. шапку модуля: его
  // читает панель прогонов), а теперь его читает и тревога о бесплодных
  // кронах. Повод конкретный: Watchdog девять прогонов подряд заканчивал
  // словами «причина пропуска не записана» — звал разбираться, не сказав куда.
  //
  // Два кода намеренно РАЗНЫЕ там, где раньше был один молчаливый исход:
  // «спрашивать было нечего» и «спросили, ответы плохи» чинятся в разных
  // местах, и слить их значило бы потерять третье состояние (§4.0).
  no_live_questions: 'за 7 дней не набралось живых вопросов туристов — прогон не о чем было проводить',
  pass_rate_below_threshold: 'судья ответил, и доля верных ответов ниже порога — чинить ответы Кузьмича',
  // Прогон упал исключением. Текст ошибки лежит в error_msg рядом; код нужен,
  // чтобы падение не выглядело пропуском без причины.
  run_threw: 'прогон упал с исключением — смотри error_msg рядом',
};
