...
export function buildEvoAlert(
  ...
  // Разбудить обязан ОТКАЗ, а не отсутствие.
  //
  // `failed` — объектив упал: состояние новое, чинимое, и молчать о нём нельзя.
  // `not_implemented` — объектива нет вовсе: состояние постоянное и осознанное
  // (объектив безопасности снят намеренно, проверки живут в CI). Оно печатается
  // в теле алерта, но поводом для самого алерта не служит.
  //
  // Найдено первым же прод-прогоном после E-3: «НЕ ПРОВЕРЕНО: 1 из 10 —
  // безопасность» приходило при пустом прочёсе и приходило бы трижды в сутки
  // вечно. Тот же волк-крикун, про которого в этом файле уже записан урок
  // EVO_FLAGSHIP_DEFERRED: напоминать несколько раз в день о принятом решении
  // значит научить не читать алерт. Различие `failed`/`not_implemented` было
  // заведено в данных, но не доведено до повода будить владельца.
  const failedLenses = unchecked.filter((l) => l.status === 'failed');
  ...
  if (nothingNew && !blind && !downgraded && !mute && failedLenses.length === 0) return null;
