...
async function analyzeFeedback(input: FeedbackInput): Promise<string | null> {
  ...
  const messages: ChatMessage[] = [
    {
      role: 'system',
      content: `Ты ведёшь обучение Evo-системы платформы TourHab. Тебе дают результат проверки человеком авто-фикса кода. Извлеки ОБОБЩАЕМЫЙ урок, который улучшит будущие фиксы той же категории.

Урок — actionable-правило, а не пересказ: "Для фиксов категории X — всегда/никогда делать Y, потому что Z".
Хороший: "Для add_indexes — проверять, нет ли уже частичного индекса на той же колонке, иначе дубль отклоняют".
Плохой (запрещён): "нужно быть внимательнее", "тестировать тщательнее", "фикс был неудачным".

Если outcome = regression/failure — назови конкретный класс ошибки, приведший к откату. Если success — что именно сработало и стоит повторять.
Ответь 1-2 предложениями на русском, с привязкой к категории фикса. Без общих слов.`,
    },
    {
      role: 'user',
      content: `Фикс: ${row.action} (категория: ${row.category})
Проблема: ${row.issue_title}
${row.diff_summary ? `Diff: ${row.diff_summary.slice(0, 500)}` : ''}

Результат проверки человеком: ${input.outcome}
Оценка влияния: ${input.impact_score} (шкала от -100 = сильная регрессия до +100 = большая польза)
Комментарий человека: ${input.human_notes}

Сформулируй обобщаемое правило для категории "${row.category}" по формату из системного промпта: "Для фиксов категории ${row.category} — делать/не делать Y, потому что Z". Опирайся прежде всего на комментарий человека.`,
    },
  ];
