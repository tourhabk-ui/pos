...
export interface PrecisionStats {
   ...
   *
   * `ignored` сюда не входит (поправка 24.08). `ignored` — это «не разбирали»
   * (миграция 912: судья не успел добраться, или владелец разом закрыл
   * очередь без индивидуального вердикта); `rejected` — «прочитали и это
   * неправда». Первое ничего не говорит о том, врёт модель или нет, а счёт
   * точности отвечает именно на этот вопрос. Смешение однажды уже обрушило
   * точность ниже PRECISION_FLOOR разовой массовой уборкой ~80 непрочитанных
   * записей — тормоз сработал не на промахе модели, а на размере очереди.
   */
  rejected: number;
}
...
/**
 * Окно точности в днях — по дате вердикта человека (`resolved_at`), решение
 * владельца 25.09.
 *
 * До этого точность считалась за всю историю, и тормоз держал запрет 30
 * прогонов подряд: 26 принятых против 66 отвергнутых, 28% при пороге 50%.
 * Но 57 из 92 вердиктов пришлись на старые находки без записанной модели
 * (до миграции 771), а у атрибутированных счёт был 18 к 17 — 51%. При одном
 * пробнике в сутки старый хвост не отпустил бы тормоз больше месяца даже при
 * сплошь принятых находках: тормоз должен замедлять, а не запирать (шапка
 * файла). Окно оставляет тормоз, но старые отказы перестают держать его вечно.
 */
export const PRECISION_WINDOW_DAYS = 60;
...
/**
 * Сколько прогонов ПОДРЯД тормоз держит allowGuesses=false — сторож самого
 * тормоза (24.08). До этого поле `precision`/`guesses_allowed` уже уходило
 * наружу из evo-report, но систематически его никто не читал: раннер только
 * печатал строку в лог джобы, и крен знаменателя точности от массовой уборки
 * очереди (миграция 912) держал тормоз замороженным несколько прогонов
 * незамеченным — сам механизм не мониторит собственное здоровье, только его
 * следствие («0 находок», которое неотличимо от «чистого кода»).
 *
 * Тот же порог, что у IDLE_RUNS_THRESHOLD (lib/agents/cron-idle.ts): один
 * просевший прогон — шум, три подряд — обрыв. `allowGuesses` при этом
 * бывает `false` ТОЛЬКО когда точность измерена (`precision !== null`) И
 * ниже порога — decidePublish() возвращает `allowGuesses: true` при малой
 * выборке (precision===null), так что стрик никогда не путает «рано судить»
 * с «тормоз держит».
 */
export const PUBLISH_GATE_STREAK_THRESHOLD = 3;

/** Чистая: следующее значение стрика по решению текущего прогона. */
export function nextPublishGateStreak(allowGuesses: boolean, prevStreak: number): number {
  return allowGuesses ? 0 : prevStreak + 1;
}
