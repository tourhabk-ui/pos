...
export interface GrowthFinding {
  ...
  model?: string | null;
  /**
   * Где сломалось и кого чинить (миграция 820, схема arXiv:2607.28802).
   * Тормоз точности смотрит на `fault_side`: отвергнутая находка про сломанный
   * деплой не свидетельствует о том, что врёт модель.
   */
  edge?: string | null;
...
/**
 * Авто-запуск руки (Эволюция 3.0, п.3). Issues с меткой `agent-proposal`
 * подхватывает claude.yml и сам ведёт до draft-PR. Метку получают ТОЛЬКО
 * находки, где у руки в Actions есть всё для фикса прямо из репо и текста
 * issue — без прод-БД и секретов:
 *
 * - «Прод 500: <роут>» — детерминированный класс п.1: file_path указывает на
 *   роут, в описании текст последней ошибки; фикс — код.
 *
 * Whitelist расширяется по одному классу, после того как класс показал себя
 * в ручном режиме. Сознательно НЕ авто: funnel и intel (расследование, а не
 * механический фикс — см. OUTWARD_CATEGORIES), Кузьмич-евал (нужны cases из
 * прод-журнала), любые модельные догадки (точность догадок — отдельная
 * подсистема с тормозом, авто-исполнять их нельзя по построению).
 *
 * Включается выключателем EVO_AUTOPR=1 (Actions variable): ключ руки
 * (ANTHROPIC_API_KEY) сейчас без кредитов, и слать метку в мёртвую руку —
 * плодить красные прогоны.
 */
export function isAutoRunnable(f: Pick<GrowthFinding, 'category' | 'title' | 'model'>): boolean {
  if (f.model !== 'deterministic') return false;
  return f.category === 'bug' && f.title.startsWith('Прод 500:');
}
