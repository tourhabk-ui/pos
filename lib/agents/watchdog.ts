...
/**
 * Деньги туриста за ОТМЕНЁННУЮ бронь лежат у нас и ждут ДЕЙСТВИЯ человека.
 *
 ...
 *
 * Решение владельца 11.09: возврат — 100%, без исключений по срокам.
 * Технический возврат через платёжный API не подключён (отдельное решение),
 * поэтому деньги переводит администратор вручную и отмечает через
 * `POST /api/admin/finance/refunds` (UI — вкладка «Выплаты», секция «Ждут
 * возврата»). Эта тревога перестаёт звенеть о конкретном платеже, когда
 * отметка сделана — не раньше, потому что до неё возврата не было НИГДЕ,
 * кроме как в намерении.
 *
  ...
  try {
    ...
    return {
      ...
      details:
        ...
        `(самая старая отмена — ${oldest} ч назад). Оператору они не уйдут; туристу положено ` +
        `100% (решение владельца 11.09) — отметьте возврат вручную: /hub/admin/finance, вкладка «Выплаты».`,
    };
  } catch (err) {
