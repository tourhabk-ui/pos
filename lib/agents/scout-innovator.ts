...
Ответь ТОЛЬКО JSON-массивом, без markdown и без текста вокруг.`,
  ...
  try {
    ...
    //
    // `deepThinking: false` — правка 12.09 после разбора панели: подъём 05.09
    // (800→3000) НЕ вылечил обрыв, он сдвинул позицию отказа с ~2440 до ~2527
    // символов — на две тысячи двести добавленных токенов ответ вырос меньше
    // чем на 100 знаков. Значит бюджет уходил не в ответ, а в РАЗМЫШЛЕНИЕ:
    // `deepThinkingBudget` калибровался по короткой задаче (575-686 знаков
    // рассуждения) и растягивается под многошаговый анализ конкурентных
    // триггеров куда шире. Здесь размышление не нужно ценой полноты: JSON
    // проверяет ОТДЕЛЬНЫЙ LLM-критик ниже (criticReviewProposal), и полный,
    // но менее «обдуманный» ответ надёжнее нуля предложений после обрыва.
    // Третий подъём голого maxTokens повторил бы ту же догадку без причины
    // ждать другого исхода — см. docstring `callAIQuality` в providers.ts.
    //
