...
--
-- ВНИМАНИЕ (22.08.2026). Этот файл НЕ накатывается на прод: деплой применяет
-- только migrations/*.sql (start.js → scripts/migrate-standalone.js). Он
-- описывает форму ДО эры нумерованных миграций, и для части таблиц эта форма
-- на проде уже другая.
--
-- Пример, стоивший двух живых поломок: tour_availability ниже объявлена как
-- tour_id UUID → tours(id), а на проде живёт форма миграции 040 —
-- operator_tour_id BIGINT → operator_tours(id). Пока реестр схемы объединял
-- оба источника, гард фантомных колонок молчал: инструмент оператора my_tours
-- падал на каждом вызове, скрейпер не записал ни одного заезда.
--
-- Теперь при расхождении старше миграция (lib/database/schema-registry.ts).
-- Правя схему, правь МИГРАЦИЮ; этот файл — историческая справка, а не истина.
