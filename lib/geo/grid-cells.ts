...
/**
 * Клетки НИЖЕ порога суши (0.02 кв.°) — поимённо и с причиной.
 *
 * Порог отсекает клетки, где край задевает угол градуса, а пакет почти
 * целиком море. Иногда в таком углу лежит то, что человек ищет на карте, и
 * тогда клетка заводится решением, а не правилом. Сторож
 * (tests/unit/grid-cells.test.ts) требует, чтобы исключение и правда было
 * ниже порога: клетка, прошедшая порог сама, из этого списка уходит.
 */
export const CELLS_BELOW_LAND_THRESHOLD: Readonly<Partial<Record<GridCellId, string>>> = {
  // 24.09, скрины владельца с /map («юг Камчатки на нашей карте очень
  // кривой») и его «да» на отдельный пакет. Южнее 51° у края остаётся
  // только кончик мыса Лопатка — 0.007 кв.° (50.865–51.0 с.ш., 156.66–156.85
  // в.д.). Замер по полигонам суши OSM, той же маске, что у океана обзора;
  // для соседней cell-51n156e этот способ даёт 0.432 против 0.437 по Natural
  // Earth, то есть сходится. Остальная суша клетки — Шумшу и Парамушир,
  // Сахалинская область: в долю края они не входят и не засчитаны.
  'cell-50n156e': 'мыс Лопатка — южная оконечность края; без клетки кончик полуострова на подробных зумах серый',
};
