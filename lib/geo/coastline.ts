 ...
 * (RADAR_MAX_KM = 500). Половина самого длинного звена контура — около 50 км,
 * поэтому звено, оба конца которого дальше 700 км, не может приблизиться к
 * центру ближе 650. Порог только экономит разметку, форму он не меняет;
 * инвариант стережёт тест radar-coastline — через сами константы, а не
 * через числа, переписанные от руки.
 *
 * До 02.09 стояло 400 при скопе 200 км. Скоп вырос впятеро по слову
 * владельца: все вулканы с повышенным кодом KVERT — Ключевской (~360 км),
 * Безымянный, Шивелуч (~440 км), Крашенинников — лежали за 200-км кольцом,
 * и на радаре «вулканов» была одна сейсмика. Порог отсечения обязан расти
 * вместе со скопом, иначе берег обрывается внутри кольца.
 */
export const CULL_KM = 700;
