...
export interface PackSafetySnapshot {
  ...
  unavailable: boolean;
  /**
   * Тип верхней тревоги (`external_alerts.alert_type`) и ЧТО ПРИ НЕЙ ДЕЛАТЬ.
   *
   * Issue #1485: «Тревоги о вулкане и землетрясении формируются, но не
   * доходят ни до кого». Снимок нёс `hasAlert`, `maxSeverity` и заголовок —
   * то есть ЧТО случилось, — а руководство оставалось онлайн. Тревога же
   * застаёт человека там, где сети нет по определению: донесли тревогу,
   * ответ на неё оставили в интернете.
   *
   * Шаги кладутся В ПАКЕТ при сборке, вместе с самой тревогой, и берутся из
   * детерминированного справочника (`lib/safety/alert-guidance`), а не у
   * модели. Пустой массив при непустом `topType` — законное «руководства для
   * этого типа не записано», а не «делать нечего».
   */
  topType?: string | null;
