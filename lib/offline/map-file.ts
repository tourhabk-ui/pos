...
/**
 * Настроен ли адрес файла карты. Пусто — значит фаза 2 ещё не подключена.
 *
 * ВЫЗОВА НЕТ НАМЕРЕННО, и это касается всего модуля: докачка по диапазонам,
 * решение о возобновлении, проверка места и сборка файла — ни одна из них не
 * подключена к экрану. Модуль целиком заготовка второго способа хранить
 * карту (единый PMTiles вместо тысяч тайлов), и включается он не отсюда, а
 * появлением `NEXT_PUBLIC_MAP_PMTILES_URL` вместе с раздачей самого файла.
 *
 * Перепись 22.08.2026 оставила модуль как честно объявленную незавершённость:
 * удалить его значит потерять вместе с ним и записанную причину, по которой
 * нынешний способ (тайлы OSM) остаётся единственным.
 */
