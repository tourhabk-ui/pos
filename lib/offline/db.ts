...
export interface FieldCheckQueueItem {
  ...
  photos: string[];
  /**
   * id проверки на сервере, если ВЕРДИКТ уже принят, а фото ещё не все
   * довезены. Раньше элемент удалялся сразу после первой попытки загрузки
   * фото — упавший снимок терялся навсегда, хотя комментарий обещал
   * «довезём в следующий заход». Теперь элемент живёт, пока в photos есть
   * недоставленное, а повторный заход шлёт только фото, не вердикт.
   */
  sentCheckId?: string | null;
  queuedAt: number;
}
