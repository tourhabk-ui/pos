 ...
 *   - В описании каждого тура: ссылка на vedar.app/hub/tour/{id}
 *
...
const SITE_URL = process.env.NEXT_PUBLIC_SITE_URL ?? 'https://vedar.app';
...
export const avitoAdapter: ChannelAdapter = {
  ...
  async pushBooking(_input: PushBookingInput): Promise<PushBookingResult> {
    ...
    // Бронирование происходит на vedar.app — ссылка зашита в описание тура.
    // Авито используется только для генерации лидов (трафик → наш сайт).
    return { success: false, error: 'Авито не поддерживает прямое бронирование — пользователи направляются на vedar.app' };
  },
