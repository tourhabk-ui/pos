...
export const CENSUS_BASELINE = {
   ...
   *
   * ── Перемер 01.09.2026: было 215, стало 6 ─────────────────────────────
   *
   * Это НЕ смягчение порога и НЕ регрессия данных. Прежние 215 были
   * измерением обещания, которого данные никогда не подтверждали.
   *
   * Замер 19.08 сделан в 23:27, род связи вошёл в код тем же утром в 07:58 —
   * правило действовало. Но РАЗМЕТКА на проде к тому часу ещё не доехала:
   * род у связей стоял неустановленный, и черта считала путевыми точками всё
   * подряд, включая «рядом». Ровно то, о чём предупреждает §4.1: миграция 167
   * завела связи как «места в 15 км от ЦЕНТРА маршрута», а платформа читала
   * их как «маршрут здесь проходит».
   *
   * Когда разметка 874 дошла до данных, «рядом» перестало считаться путём — и
   * стало видно настоящее число. Связей рода `waypoint` во всей базе 134
   * штуки: даже в идеале это потолок в 67 маршрутов, а после вычета
   * конфликтов линии с точкой и облётов остаётся горстка.
   *
   * ── Что тут на самом деле сломано ─────────────────────────────────────
   *
   * Не линии. Снятой геометрии 272 из 288, улика записи есть у ~277 линий —
   * скрейп idilesom вернул себе право вести по уликам 18.08, и заслуженно.
   * Сломана вторая половина: сверять линию не с чем.
   *
   * Исключение «линия доказана сама» (решение A) не срабатывает, потому что
   * у 282 маршрутов НЕ ЗАПИСАН донор — страница-источник, откуда линия
   * пришла (`anchor_sources: {"не записан": 282}`). Скрейпер её принёс и не
   * сохранил происхождение.
   *
   * Поэтому 6 — честная отметка сегодняшнего дня, а работа не в этом файле:
   * записать донора у скрейпленных маршрутов и разметить путевые точки. Как
   * только это сделано, число вырастет само, и вот тогда база меряется заново.
   */
  navigable: 6,
