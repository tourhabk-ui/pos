...
export interface NavigabilityInput {
  ...
  /**
   * Линия доказана тремя уликами и может вести БЕЗ путевых точек.
   *
   * Решение A плана (Ф2). Считает не черта, а lib/routes/trust-decision:
   * там собраны все три улики и их свежесть. Сюда приходит готовый ответ,
   * потому что черта — про обещание, а не про хранение доказательств.
   *
   * Снимает РОВНО ОДИН отказ: «линию не с чем сверить». Все прочие — набросок,
   * неподтверждённое происхождение, расхождение точек, разрыв линии, облёт —
   * остаются в силе: три улики говорят о линии, а не о том, что по ней можно
   * идти пешком.
   */
  lineProvenWithoutWaypoints?: boolean;
  /**
...
export function routeNavigability(i: NavigabilityInput): Navigability {
  ...
  if (pathPoints.length < MIN_ROUTE_WAYPOINTS && !i.lineProvenWithoutWaypoints) {
    ...
    //
    // Исключение — решение A (19.08): линия, у которой есть запись прибора,
    // свежее совпадение с оригиналом и подтверждённый донор, доказана САМА,
    // и требовать сверх этого две точки значит требовать разметку ради
    // разметки. Отказ снимается только при всех трёх уликах разом — их
    // совместность проверяет lib/routes/trust-decision.
