...
// создаётся там, где нужен. Держатель `botInstance` пережил ту чистку и
// девятнадцать дней стоял мёртвой строкой — его никто не читал и не писал.
// Нашёл линтер (prefer-const), а не человек: переменная, которую невозможно
// переприсвоить, — это и есть след удалённого вокруг неё кода.
