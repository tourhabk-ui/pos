...
/**
 * Текст ответа MCP — из РАЗОБРАННОГО JSON, а не из сырого тела.
 *
 * Восемь суток (12-19.08) issue #1155 сообщала «канал mcp не отдал ни одного
 * тура», и всё это время туры отдавались. Ломалась сама проверка.
 *
 * В сыром теле перенос строки записан как `\n` — ДВА символа, обратный слэш и
 * буква `n`. Каталог печатает каждый тур с новой строки, поэтому перед `ID`
 * оказывалась буква, границы слова не было, и `\bID(\d+)\b` не совпадал ни
 * разу. В разобранном тексте там настоящий перенос — и совпадает всё.
 *
 * Диагностика при этом печатала РАЗОБРАННЫЙ текст и потому показывала `ID27`
 * прямо в строке «ID туров не найдены». Два места читали разное; теперь оба
 * читают это.
 */
export function mcpText(body: string): string {
  const t = body.trim();
  if (t === '') return '';
  try {
    const obj = JSON.parse(t) as { result?: { content?: Array<{ text?: string }> } };
    const said = (obj.result?.content ?? []).map((c) => c.text ?? '').join('\n').trim();
    // Разобралось, но текста нет — отдаём пустоту, а не сырое тело: иначе
    // поиск пойдёт по служебным полям JSON и найдёт что-нибудь постороннее.
    return said;
  } catch {
    // Не JSON — ищем по тому, что пришло. Отличить это от исправного ответа
    // умеет mcpDiagnosis, здесь важно не потерять содержимое.
    return t;
  }
}
...
export function mcpTourIds(body: string): Set<string> {
  ...
  while ((m = re.exec(mcpText(body))) !== null) out.add(m[1]);
  return out;
}
...
export function mcpDiagnosis(body: string): string | null {
  ...
  // ID ищутся здесь же, а не принимаются на веру от вызывающего.
  //
  // Прежде эта ветка возвращала «ID не найдены» БЕЗ проверки — просто как
  // последнюю. Пока функцию звали только при пустом наборе, это сходило с рук,
  // но утверждение было ничем не обеспечено: диагностика 19.08 напечатала «ID
  // туров не найдены» и тут же процитировала ID27. Проверка обязана смотреть.
  if (mcpTourIds(text).size > 0) return null;

  // Текст есть, отказа нет, а ID действительно не нашлись — значит изменился
  // формат строк, и сломана ПРОВЕРКА, а не платформа. Это тоже надо называть
  // вслух.
