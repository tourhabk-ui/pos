...
/**
 * Дата-снапшот в хвосте id: `-2025-09-23`, `-20250923`, `-2025-09`.
 *
 * Вырезается ЦЕЛИКОМ до разбора версии. Прежде отсекался только год
 * (числа ≥ 100), и это выглядело достаточным — пока проба 165 не сняла
 * реальный список из 162 моделей DashScope. Там `qwen3-max-2025-09-23`
 * дал числа 3, 9 и 23: год отфильтровался, месяц и день — нет, и версией
 * стало ЧИСЛО МЕСЯЦА. Двадцать три больше, чем 3.8 у `qwen3.8-max`, и
 * годовалый снапшот побеждал флагман.
 *
 * Цена ошибки — не academic: этим же резолвером выбирается модель для
 * решателя эволюции и для ПУБЛИЧНЫХ текстов. В шапке providers.ts уже
 * описан прошлый случай той же болезни («быстрая мелкая модель выигрывала
 * гонку, и публичные тексты писала она»). Это второй, другой механизм.
 *
 * Вырезаем только явные формы даты с ведущим 20xx — чтобы не задеть
 * настоящие номера версий вроде `qwen3.5-397b-a17b`.
 */
const DATE_SNAPSHOT = /[-_]20\d{2}(?:[-_]\d{1,2}){0,2}(?![\d.])|[-_]20\d{2}[01]\d[0-3]\d(?![\d.])/g;

/** Максимальная «версия» из id, игнорируя дату-снапшот и годы. */
function versionScore(id: string): number {
  const nums = (id.replace(DATE_SNAPSHOT, '').match(/\d+(?:\.\d+)?/g) ?? [])
    .map(Number)
    .filter((n) => n > 0 && n < 100);
