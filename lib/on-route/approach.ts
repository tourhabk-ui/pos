...
/**
 * Радиус, внутри которого человека можно считать УЖЕ НА МАРШРУТЕ.
 *
 * Кокпит при первом фиксе выбирает, к какой точке вести. Раньше он всегда брал
 * ближайшую — без вопроса, на маршруте человек или нет. Замысел был здравый:
 * тот, кто стоит посреди тропы, не должен получать «до точки 1 — сорок
 * километров назад».
 *
 * Но у правила не было условия. Полевой скрин 17.08, «Вулкан Козельский»:
 * человек в Петропавловске, до маршрута десятки километров, ближайшей из трёх
 * точек случайно оказалась последняя — и экран написал «3 из 3». Прибор
 * объявил пройденным то, к чему ещё не подошли: прогресс полный, точек впереди
 * нет, а идти — всё.
 *
 * «Ближайшая точка» значит «продолжайте отсюда» и имеет смысл, только когда
 * человек действительно на маршруте. Километр — расстояние, с которого точку
 * видно и на которое сходят с тропы; четырнадцать километров — это другой
 * маршрут, другой день и, чаще всего, город.
 *
 * Дальше этого радиуса маршрут начинается с начала, а путь от человека до
 * первой точки показывается подходом — тем, чем он и является.
 */
export const ON_ROUTE_ENTRY_KM = 1;
...
/**
 * На чём держится вывод «человек ещё не на маршруте».
 *
 *  - `off_line` — измерено: до линии дальше радиуса входа (ON_ROUTE_ENTRY_KM).
 *  - `farther_than_route` — выведено арифметикой: до следующей точки дальше,
 *    чем длина ВСЕГО маршрута. Стоя на маршруте, так оказаться нельзя —
 *    расстояние до любой его точки не превышает его длины. Порога здесь нет
 *    вовсе, и придумывать его не потребовалось: противоречие в самих данных.
 */
export type NotOnRouteBasis = 'off_line' | 'farther_than_route';

export interface NotOnRoute {
  basis: NotOnRouteBasis;
  /**
   * До линии по прямой, км. Известно только при `off_line`: при
   * `farther_than_route` трека нет вовсе, мерить не по чему, и здесь null —
   * «не знаю» вместо выдуманного числа (§4.0).
   */
  approachKm: number | null;
}

/**
 * Человек ещё НЕ на маршруте — и потому пешая оценка времени к нему не
 * относится.
 *
 * Скрин владельца 13.09 (#1847): «138.0 км до следующей точки», «~39 ч 25
 * мин» и рядом «всего 64.3 км». Каждое число правдиво по отдельности: до
 * точки считается ОТ ЧЕЛОВЕКА (approach.totalKm = подход + вдоль тропы +
 * выход), «всего» — длина самой линии. Вместе они читаются как
 * бессмыслица, потому что прибор не различал два положения и оба показывал
 * одинаково.
 *
 * Хуже цифры был расчёт времени: 39 часов получены пешей моделью, применённой
 * к 137 километрам ПО ПРЯМОЙ через край. Предпосылка модели («человек идёт по
 * тропе») там не выполняется, а число выглядит точно так же, как настоящее.
 * Это тот же класс ошибки, что уверенная стрелка компаса при мёртвом датчике:
 * пустой прибор заставляет достать карту, врущий — нет.
 *
 * Порог не изобретается: `ON_ROUTE_ENTRY_KM` уже решён и уже описан выше как
 * «радиус, внутри которого человека можно считать УЖЕ НА МАРШРУТЕ», а вторая
 * ветвь вовсе обходится без порога.
 *
 * Отсутствие улик — НЕ «человек на маршруте». Когда трека нет и длина
 * маршрута неизвестна, функция возвращает null: это «не знаю», и экран
 * ведёт себя как прежде, а не объявляет человека сошедшим с пути.
 */
export function notOnRoute(input: {
  /** План подхода — есть, когда у маршрута есть линия. */
  plan: ApproachPlan | null;
  /** Прямая до следующей точки, км — когда линии нет и плана тоже. */
  straightToNextKm: number | null;
  /** Длина всего маршрута по плечам, км. */
  routeTotalKm: number | null;
}): NotOnRoute | null {
  const { plan, straightToNextKm, routeTotalKm } = input;

  if (plan && Number.isFinite(plan.approachKm)) {
    return plan.approachKm > ON_ROUTE_ENTRY_KM
      ? { basis: 'off_line', approachKm: plan.approachKm }
      : null;
  }

  if (
    typeof straightToNextKm === 'number' && Number.isFinite(straightToNextKm) &&
    typeof routeTotalKm === 'number' && Number.isFinite(routeTotalKm) && routeTotalKm > 0 &&
    straightToNextKm > routeTotalKm
  ) {
    return { basis: 'farther_than_route', approachKm: null };
  }
