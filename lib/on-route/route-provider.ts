 ...
 * (владелец 28.08, PR 5B-1: инфраструктура; 28.08 — «собираем свой»,
 * подключение реального провайдера).
 *
 * Владелец зафиксировал для первого релиза: car — да, foot — только по
 * известной сети (5B-2, отдельно), маршрутизатор — только через серверный
 * адаптер (ключ/провайдер не в браузере). Региональный тест 28.08
 * (публичный демо-OSRM, реальные координаты Камчатки) зафиксировал
 * нормализованную форму ответа (lib/on-route/calculated-route.ts) — этот
 * тест и остался только источником формы; внешний провайдер (Yandex/2ГИС
 * bake-off) от рассмотрения ОТКЛОНЁН в тот же день в пользу уже
 * существующей инфраструктуры: `roadGraphCarProvider`
 * (lib/on-route/road-graph-car-provider.ts) поверх своего дорожного графа
 * Камчатки (миграция 760, решение владельца 2026-07-20). Ни ключей, ни
 * лицензии, ни трансграничной передачи — весь расчёт на наших данных.
 *
 * `roadGraphCarProvider` — реализация, подключённая к `/api/routes/build`
 * СЕГОДНЯ. `notWiredCarRouteProvider` остаётся в файле как честный фолбэк
 * (не молчит и не выдаёт заглушку за реальный маршрут) на случай, если
 * провайдер понадобится временно отключить, и как опора существующих
 * тестов. `fakeCarRouteProvider` и `fakeFarSnapCarRouteProvider` — тестовые
 * адаптеры на РЕАЛЬНЫХ данных пробы 28.08 (не в проде, только для тестов):
 * доказывают, что found/not_found/snap-guard работают независимо от того,
 * какой конкретно провайдер подключён.
 */
