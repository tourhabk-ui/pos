...
/**
 * Отметка СБОРКИ, а не запуска.
 *
 * 04.09 два замера подряд ушли впустую: шаг «дождись деплоя» смотрел на
 * uptime процесса и ловил ЛЮБОЙ свежий рестарт — в том числе от предыдущего
 * мержа, выкатывавшегося в те же минуты. Прогон уходил на старом коде и
 * отвечал на вчерашний вопрос, причём выглядел совершенно здоровым (run 9:
 * таймаут ровно 15 с и ни одной новой строки — то есть код был прежний).
 *
 * uptime отвечает на вопрос «давно ли перезапускались», а спросить надо
 * «этот ли код сейчас на проде». Время сборки вычисляется здесь, в момент
 * `next build`, и уезжает в образ: оно строго растёт от деплоя к деплою, и
 * его достаточно, чтобы сверить с временем коммита.
 */
