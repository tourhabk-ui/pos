...
Источник высот — Copernicus GLO-90 (ESA), лежит на AWS Open Data без
...
GLO-90 — это 90 метров на отсчёт. На широте Авачинской группы:

    z10 -> 91.6 м/пиксель   <- здесь данные ЗАКАНЧИВАЮТСЯ
    z11 -> 45.8 м/пиксель
    z12 -> 22.9 м/пиксель
    z14 ->  5.7 м/пиксель

Запечь z14 технически можно — интерполятор нарисует гладкие склоны с
точностью 5.7 м из данных с шагом 90 м. Это ровно тот дефект, из-за которого
...
Поэтому: MAXZOOM = 12 — два уровня билинейного сглаживания сверх родного,
чтобы тень не рассыпалась ступенями, и ни одного уровня сверх того. Дальше
MapLibre растягивает сам (overzoom), и это видно как мягкость, а не как
ложная детализация. Метаданные архива несут `native_zoom` = 10, чтобы
следующий читатель не принял 12 за разрешение источника.
...
# Copernicus GLO-90 на AWS Open Data. Без ключа и без регистрации — в отличие
...
def dem_tile_name(lat: int, lng: int) -> str:
    """Имя тайла GLO-90 по левому нижнему углу градусной клетки."""
    ns = 'N' if lat >= 0 else 'S'
    ew = 'E' if lng >= 0 else 'W'
    return (f'Copernicus_DSM_COG_30_{ns}{abs(lat):02d}_00_'
            f'{ew}{abs(lng):03d}_00_DEM')


def fetch_dem_tiles(bbox, cache_dir):
    """Скачивает клетки GLO-90, покрывающие bbox. Возвращает список путей."""
        ...
        for lng in range(math.floor(west), math.ceil(east)):
            ...
            if not os.path.exists(path):
                ...
                try:
                    ...
                    # Клетка может отсутствовать законно: над океаном GLO-90
                    # публикует не все квадраты. Это «нет данных», а не сбой —
...
def build_mosaic(paths, bbox):
    ...
    west, south, east, north = bbox
    # Шаг берём у первой клетки — у GLO-90 он одинаков в пределах широтной
    # полосы, а регион пробы в одну полосу и укладывается.
    ...
    with open(args.out, 'wb') as f:
        ...
        w.finalize(
            ...
            },
            {
                ...
                'source': 'Copernicus GLO-90 (ESA), AWS Open Data',
