...
| Glassmorphism на светлых сплошных фонах; стекло на SOS и confirmation-диалогах | решение владельца 2026-09-26 (пересматривает 2026-08-15): поверх фото, карт, рельефных текстур и тёмных подложек стеклом может быть всё, включая приборы (компас, лист с главной цифрой) и предупреждения (кромка `--warning`) — `backdrop-blur-md bg-black/40 border border-white/15 rounded-2xl`, плотнее `.fx-glass-dense`; непрозрачны только SOS и confirmation; содержимое стекла — `data-theme="dark"` или `--glass-fg`; `prefers-reduced-transparency` → фолбэк на `var(--bg-card)` |
