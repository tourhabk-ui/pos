  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', margin: 0, lineHeight: 1.5 }}>
        {/* Честно: сосед ретранслирует, только пока у НЕГО есть связь (или
            когда она у него появится — store-and-forward). Обещание «даже
            без интернета» браузерный меш дать не может: без сети связи не
            устанавливаются вовсе. Для полностью офлайн-передачи есть
            QR-эстафета на экране SOS. */}
        {hasRelay && 'Если твоя связь оборвётся — SOS передаст сосед по группе, у которого она осталась.'}
        {isConnected && peerCount === 0 && 'Других устройств группы не видно. SOS идёт напрямую; без сети — QR-эстафета выше.'}
