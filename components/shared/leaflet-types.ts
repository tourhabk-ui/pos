...
export interface MapMarker {
  ...
  /**
   * Нарисовать ТОЛЬКО геометрию, без пина.
   *
   * Одна запись маршрута — один `geometry`, поэтому линия из нескольких
   * кусков требует нескольких маркеров, и каждый ставил пин. На превью
   * снятого трека (очередь треков в хабе) запись из трёх кусков давала пять
   * пинов подряд — они не значат ничего, а читаются как точки маршрута.
   *
   * Умолчание — прежнее поведение: пин рисуется. Флаг ставит тот, кто знает,
   * что его маркер — держатель линии, а не место.
   */
