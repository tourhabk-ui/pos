...
export default function VedarMap({
  ...
  useEffect(() => {
    ...
    // Новая карта — новое «вот вы». Смена темы или района пересоздаёт
    // карту в стартовой точке, а флаг от прежней оставался поднятым, и
    // первый фикс её уже не центрировал (26.09: «открывает вулкан
    // Авачинский» вместо места человека).
  ...
  // (см. buildRegionOverlay): рельеф, океан и наши места — сразу, горизонтали
  // и OSM — с DETAIL_MIN_ZOOM. Вершины и посёлки OSM шли в базовом ярусе до
  // 13.09, пока владелец не убрал их с карты совсем.
  // Добавленное не снимается: MapLibre держит источники
