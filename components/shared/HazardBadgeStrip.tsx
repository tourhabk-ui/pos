'use client';
import {
  AlertTriangle, Wind, Mountain, Thermometer, Flame,
  Waves, Eye, CloudLightning, Signal, Users,
} from 'lucide-react';
import { HAZARD_LABELS } from '@/components/places/types';

const HAZARD_SEVERITY: Record<string, 'danger' | 'warning' | 'ocean'> = {
...
export const HAZARD_SEVERITY_COLORS: Record<'danger' | 'warning' | 'ocean', string> = {
  danger: 'var(--danger)',
  warning: 'var(--warning)',
  ocean: 'var(--ocean)',
};
  ...
  return (
    <div className={`flex flex-wrap gap-2 ${className}`}>
      {hazards.map(h => {
        ...
        const label = HAZARD_LABELS[h]?.label ?? h;
        return (
          <span
            key={h}
