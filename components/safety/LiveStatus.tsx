    ...
    <div className="radar">
      <div className="scope">
        ...
        {placed.length === 0 && (
          ...
          //
          // Прежняя подпись обещала два источника — «Сейсмики и тревог вулканов
          // рядом нет», — а радар знал один: вулканы с повышенным кодом KVERT.
          // Предупреждения МЧС и сейсмособытия на круг не попадали вовсе.
          // Полевой случай 10.08: в 70 км от человека Мутновский с активным
          // предупреждением «не приближаться», а круг пуст. Формально про свой
          // источник радар не врал; беда в том, что читают его не про источники.
          //
          // Сбой источников назван отдельно: пустой круг после упавшего запроса
          // и пустой круг в спокойный день — разные вещи.
          <div className="clean">
            {degraded ? 'Часть источников не ответила — показано не всё' : 'Опасностей рядом не зафиксировано'}
          </div>
        )}
      </div>
