...
export TOURHAB_URL="https://vedar.app"  # default
```
...
curl https://vedar.app/api/agents/board-meeting/preflight \
  -H "Authorization: Bearer $JWT"

# Запустить совещание (CRON)
curl https://vedar.app/api/cron/board-meeting?secret=$CRON_SECRET

# ИЛИ запустить совещание (ADMIN JWT)
curl -X POST https://vedar.app/api/agents/board-meeting \
  -H "Authorization: Bearer $JWT" \
  -H "Content-Type: application/json" \
  -d '{"topic":"Test Run"}'

# Мониторить в отдельном терминале
curl -s https://vedar.app/api/agents/board-meeting/debug?format=stream | jq '.'
```
...
```bash
# Проверь соединение
curl https://vedar.app/api/agents/board-meeting/preflight | jq '.checks[] | select(.name=="Database")'
...
```bash
# Текущий снимок
curl https://vedar.app/api/agents/board-meeting/debug | jq '.stats'

# SSE поток (real-time)
curl -s https://vedar.app/api/agents/board-meeting/debug?format=stream | jq '.'

# Очистить буфер
curl https://vedar.app/api/agents/board-meeting/debug?format=clear
```
