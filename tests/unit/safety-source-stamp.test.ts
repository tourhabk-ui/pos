...
describe('каждая отметка — от своего источника', () => {
  ...
  it('у вулканов — своя отметка, у сейсмики — своя', () => {
    // 15.09: у вулканов отметок стало ДВЕ (наблюдение КВЕРТ и наш опрос —
    // см. volcano-two-stamps.test.ts), и печатаются они уже не через
    // stampLabel. Требование этого сторожа от того не изменилось: время
    // вулканов приходит из среза вулканов, время сейсмики — из среза
    // сейсмики. Привязываться к написанию значило бы держать формулировку,
    // а не правило.
    expect(CLIENT).toMatch(/live\.volcanoes\.updatedAt/);
