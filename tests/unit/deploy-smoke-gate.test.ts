...
describe('смоук проверяет три вещи, а не факт ответа сервера', () => {
  ...
  it('фикстура читается из обеих вкладок панели', () => {
    // 17.08 владелец завёл её дважды, и оба раза прогон видел пустоту: на
    // одной странице настроек две вкладки — Secrets и Variables. Id маршрута
    // не секрет, прятать его не от кого, и настаивать на «правильной» вкладке
    // ценой ещё одного красного прогона — упрямство, а не строгость.
    expect(DEPLOY).toMatch(/vars\.SMOKE_ROUTE_ID \|\| secrets\.SMOKE_ROUTE_ID/);
  });
  ...
  it('качество выдачи измеряется отдельно и падает на нуле пригодных', () => {
    ...
    expect(SMOKE).toMatch(/кандидаты в фикстуру/);
  });
