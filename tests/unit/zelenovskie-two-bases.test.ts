...
describe('профиль безопасности у обеих', () => {
  ...
  it('вставка профиля проверяет, что МЕСТО существует — урок 30.08', () => {
    // Миграция падала девять деплоев подряд на fk_lsp_place: гард профиля
    // смотрел на таблицу профилей («такого ещё нет» — правда), но не на то,
    // есть ли место, к которому профиль привязывается. Пока места вставляла
    // эта же миграция, разницы не было; как только их завела другая (918/919),
    // гард по имени подавил вставку мест, а профили полезли на несуществующий
    // ark_id и уронили транзакцию.
    //
    // Гард обязан проверять условие, от которого зависит УСПЕХ ВСТАВКИ,
    // а не соседнее и похожее.
    const profiles = CODE.split('INSERT INTO location_safety_profile').slice(1);
    expect(profiles.length).toBe(2);
    for (const p of profiles) {
      expect(p, 'вставка профиля не проверяет наличие места — вернётся падение по fk_lsp_place')
        .toMatch(/EXISTS\s*\(\s*SELECT 1 FROM places WHERE ark_id::text =/);
    }
  });
