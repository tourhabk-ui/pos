...
describe('подпись строки ленты', () => {
  ...
  /**
   * Живой случай 05.08: владелец прислал ленту — «действует до 1 июл.» пятого
   * августа, вывод «лента не удаляет старые». Запись при этом действующая
   * (иначе не прошла бы expires_at > NOW()), но подпись читалась как прошлое.
   * Две отдельные причины, обе в подписи, а не в фильтре.
   */
  describe('срок не врёт про себя', () => {
    it('дата другого года печатается с годом', () => {
      // «1 июл.» в августе неотличимо от прошедшего.
      const nextYear = new Date().getFullYear() + 1;
      const stamp = alertStamp({
        type: 'road_closure',
        at: '2026-07-10T09:00:00Z',
        until: `${nextYear}-07-01T00:00:00Z`,
      });
      expect(stamp).toContain(String(nextYear));
    });

    it('наш технический TTL не выдаётся за срок действия', () => {
      // Дорожным ограничениям ingest ставит ровно неделю жизни ЗАПИСИ
      // (seismic-parser: expires_hours = 24 * 7). МЧС таких дат не сообщает,
      // и печатать их как «действует до» — обещание знания, которого нет.
      const stamp = alertStamp({
        type: 'road_closure',
        at: '2026-07-10T09:00:00Z',
        until: '2026-07-17T09:00:00Z', // +168 часов = наш TTL
      });
      expect(stamp).not.toContain('действует до');
      expect(stamp).toContain('сообщение от');
    });

    it('срок из источника по-прежнему показывается как срок', () => {
      const stamp = alertStamp({
        type: 'road_closure',
        at: '2026-07-10T09:00:00Z',
        until: '2026-09-01T00:00:00Z', // не совпадает ни с одним TTL
      });
      expect(stamp).toContain('действует до');
    });
  });
