...
describe('свёрнутая лента не показывает обрывков', () => {
  it('в бегущем режиме описание — одна строка с многоточием', () => {
    // Скриншот владельца 08.08: маска окна резала многострочное описание
    // посередине — над «Вилючинским перевалом» висела одинокая строка
    // «полотно…» (хвост записи о реконструкции). В свёрнутом режиме описание
    // клампится в строку; полный текст — в раскрытом состоянии (тап).
    expect(LIVE).toMatch(/\.ticker\.scroll:not\(\.open\) \.adesc\{white-space:nowrap;overflow:hidden;text-overflow:ellipsis/);
  });
});
