...
describe('витрина не врёт цифрой', () => {
  ...
  it('29.08: «где чаще всего уходят» ранжирует по доле ухода, не по просмотрам, и не выдумывает причину', () => {
    // Отдельный запрос, не подмена существующего топа по views — иначе
    // проблемные (но не самые популярные) страницы никогда не всплывут.
    expect(API).toContain('top_exit_pages');
    expect(API).toMatch(/ORDER BY \(COUNT\(\*\) FILTER \(WHERE session_id IS NOT NULL AND next_id IS NULL\)\)::numeric \/ COUNT\(\*\) DESC/);
    // Порог выше, чем у общего топа поведения (3): на малой базе один
    // случайный визит даёт 100% ухода и топ становится шумом, а не сигналом.
    expect(API).toContain('HAVING COUNT(*) >= 8');
    expect(PAGE).toContain('Где чаще всего уходят');
    expect(PAGE).toContain('top_exit_pages');
    // Честная граница: показывает ГДЕ/КАК БЫСТРО, не придумывает ПОЧЕМУ —
    // причина без опроса/session-replay из этих данных не выводится (§4.0).
    expect(PAGE).toMatch(/не ПОЧЕМУ/);
  });
