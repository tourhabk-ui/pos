...
describe('доступ к аудиту', () => {
  ...
  it('перепись не отвечает чужим кодом', () => {
    /**
     * Прогон 18.08 01:14 вернул успех и цифры ПРЕЖНЕГО контейнера: мерж был в
     * 01:05, сборка Timeweb идёт минут десять. Внешне это выглядело как ответ
     * — блоки, счётчики, зелёный прогон, — а мы смотрели в прибор, которого
     * ещё не существовало. Ровно тот класс, ради которого весь вечер писались
     * сторожа: ответ, неотличимый от молчания.
     *
     * Версия формы отдаётся и в теле 401 — ожидание не требует секрета и не
     * запускает тяжёлую перепись на каждую попытку.
     */
    expect(API).toMatch(/AUDIT_SHAPE_VERSION/);
    const unauth = API.slice(API.indexOf('Unauthorized') - 200, API.indexOf('Unauthorized') + 120);
    expect(unauth).toMatch(/v: AUDIT_SHAPE_VERSION/);
    // Прогон обязан ДОЖДАТЬСЯ своей версии и покраснеть, если не дождался.
    expect(WORKFLOW).toMatch(/expect_v/);
    expect(WORKFLOW).toMatch(/Цифры старого кода — не ответ/);
  });
