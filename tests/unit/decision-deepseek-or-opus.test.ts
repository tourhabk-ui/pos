...
describe('Qwen: в решатель не вернулся, в текстовые пути вернулся (24.09)', () => {
  it('tools-цикл Кузьмича снова идёт через Qwen — но не в решателе', () => {
    // 08.09 снят («qwen не используем», ключ отвергнут), 24.09 возвращён
    // решением владельца «верни Qwen в текстовые пути». Решатель — отдельное
    // решение 04.08 («дипсик либо опус»), и возврат его не отменяет: проверка
    // «Qwen из решателя убран» выше стоит по-прежнему.
    expect(src).toMatch(/export async function callQwenWithTools\(/);
  });

  it('одиночный callQwen жив: на нём проба ключа (зрение) и первая фаза разведчика', () => {
