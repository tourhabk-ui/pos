...
describe('saveEvent не плодит строки с одинаковым содержимым', () => {
  ...
  it('дедуп срабатывает ДО вставки и завершает сохранение как skipped', () => {
    ...
    // Окно расширено с 700 до 1400 знаков (13.09): между условием и return
    // добавилась переоценка разряда. Проверка не про длину — про то, что
    // ветка дедупа кончается ТЕМ ЖЕ исходом; число здесь лишь ограничивает
    // поиск ближайшим `return`, чтобы совпадение не уехало в соседнюю ветку.
    expect(body).toMatch(/\(dup\.rowCount \?\? 0\) > 0\)\s*\{[\s\S]{0,1400}?return 'skipped';/);
  });
});
