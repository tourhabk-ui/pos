...
describe('версионный маркер: сборка действительно его пишет', () => {
  ...
  it('ссылки .git попадают в контекст сборки', () => {
    ...
    //
    // 30.08: прежняя версия чинила это исключением `.git` целиком плюс
    // `!.git/HEAD`/`!.git/refs/**`/`!.git/packed-refs` — и не работала: Docker
    // не может «продырявить» уже исключённую родительскую директорию, сама
    // .git никогда не обходится, и HEAD/refs всё равно не попадали в образ.
    // Проверяем действующий инвариант — .git САМА не исключена (значит её
    // содержимое, включая HEAD/refs/packed-refs, по умолчанию видно), а
    // исключены только конкретные тяжёлые части, не нужные resolveHeadSha().
    expect(
      DOCKERIGNORE,
      'строка `.git` (без подпути) исключает всю директорию — под ней HEAD/refs не восстановить никаким `!`',
    ).not.toMatch(/^\.git$/m);
    expect(DOCKERIGNORE).toMatch(/^\.git\/objects$/m);
  });
});
