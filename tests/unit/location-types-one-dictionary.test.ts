...
describe('список типов мест один', () => {
  ...
  it('союз ключей сохранён: что знал хоть один список, знает общий', () => {
    // Список — перепись факта: ключ входит сюда, потому что он был хоть в
    // одной из сведённых копий, а не потому, что показался уместным. Равенство
    // длин ниже и держит это: выдуманный ключ покраснеет.
    //
    // `cave` пришёл из ШЕСТОЙ копии — lib/places/type-label.ts (19.09).
    // Сведение пяти списков создало шестой экземпляр того же смысла, уже
