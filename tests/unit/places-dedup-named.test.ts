...
/**
 * Исходник без комментариев. Гард, ищущий запрещённую конструкцию, обязан
 * смотреть на код: иначе он падает от абзаца, который эту конструкцию
 * ОБЪЯСНЯЕТ. Здесь это случилось сразу — на комментарии про `::uuid[]`.
 */
...
describe('список пар проверяется до базы', () => {
  ...
  it('одно место нельзя слить дважды', () => {
    const p = pairListProblems([
      { keep: A, merge: C },
      { keep: B, merge: C },
    ]);
    expect(p[0]).toContain(C);
  });

  it('НЕСКОЛЬКО merge в один keep — законно', () => {
    // У Козельского три записи: «Козельский», «Вулкан Козельский», «Под
    // Козельский вулкан», в 69 и 85 метрах. Запрет собрать их за один заход
    // заставлял бы гнать три запроса подряд и наблюдать промежуточные
    // состояния — правило было строже смысла.
    expect(
      pairListProblems([
        { keep: A, merge: B },
        { keep: A, merge: C },
...
describe('тип id не утверждается, а обходится', () => {
  it('пары сравниваются как текст', () => {
    // `places.id` в боевой базе — TEXT (UUID лежит в `ark_id`). Первый вариант
    // брал `unnest($1::uuid[])` и получил с прода «operator does not exist:
    // text = uuid». Ровно та же ошибка, что три месяца роняла alert-prune,
    // где `::uuid[]` встретился с BIGSERIAL.
    expect(CODE).toMatch(/unnest\(\$1::text\[\], \$2::text\[\]\)/);
