 ...
 * (замороженная строка — в `issue-hand-model-pinned.test.ts`). Здесь
...
/**
 * Свипер не чинит одну ветку в несколько заходов (разбор расходов 07.09).
 *
 * Замер утра 06.09: шесть свипов подряд, все на `main`, 112 минут агента —
 * около 40% дневного счёта. Очередь их не экономила, а растягивала: каждый
 * следующий стартовал после предыдущего и чинил состояние, которое тот,
 * возможно, уже починил.
 */
describe('свипер CI: побеждает последнее падение', () => {
  const SWEEPER = readFileSync(join(WF_DIR, 'ci-sweeper.yml'), 'utf-8');

  it('группа конкурентности — по ветке', () => {
    expect(SWEEPER).toMatch(/group:\s*ci-sweep-\$\{\{\s*github\.event\.workflow_run\.head_branch/);
  });

  it('перекрытый свип отменяется, а не встаёт в очередь', () => {
    expect(
      SWEEPER,
      'cancel-in-progress: false выстраивает свипы в очередь — то же по деньгам, только дольше',
    ).toMatch(/group:\s*ci-sweep-[^\n]*\n\s*cancel-in-progress:\s*true/);
  });
});
