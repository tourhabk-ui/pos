...
describe('у каждого раннего выхода своя причина', () => {
  // `ai_factcheck_failed` убран из списка 29.08 и заменён точными кодами.
  // Он сваливал в одно ДВЕ разные беды: отказ судьи (чинить провайдеров или
  // промпт) и оставшуюся выдумку (чинить содержание). Для молчащего канала
  // это была единственная подсказка, и она указывала в никуда.
  //
  // Из словаря SKIP_REASON_LABELS код не удалён намеренно: им подписаны
  // старые строки журнала прогонов, и разбор должен уметь их прочесть. Но
  // НОВЫХ присвоений быть не должно — это стережёт scout-judge-reason-kept.
  const CODES = [
    'ai_channel_not_configured',
    'ai_no_items',
    'ai_synthesis_null',
    'ai_unsourced_percents',
    'ai_unsupported_claims',
