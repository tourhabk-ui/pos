...
describe('устройство обработчика', () => {
  ...
  /**
   * Здесь стояла проверка «пересчёт комиссии внутри транзакции, а не после
   * неё»: `expect(post).toMatch(/client\.query\(`SELECT recalculate_commission/)`.
   *
   * Она замораживала МЕСТО вызова и не спрашивала, что этот вызов делает. А
   * делал он не то, что обещает имя: не пересчитывал комиссию по выплачиваемым
   * платежам, а перезаписывал `partners.commission_current` — ставку для всех
   * БУДУЩИХ начислений — по лестнице за объём завершённых броней. При
   * назначенных владельцем 10% нажатие «выплатить» у оператора с десятью
   * завершёнными бронями уводило платформу на 7%, с пятьюдесятью — на 5%.
   *
   * Вызов убран (разбор 11.09), и проверка заменена на обратную: его тут быть
   * не должно. Довод прежнего комментария («иначе при откате выплаты комиссия
   * осталась бы посчитанной по невыплаченным платежам») снят вместе с вызовом:
   * функция не трогает ни `operator_commissions`, ни `tour_payments`, и
   * откатывать в ней нечего.
   *
   * Полное правило и оба места, где вызов стоял, — в
   * `tests/unit/commission-rate-decided.test.ts`.
   */
  it('ставку комиссии выплата не меняет: recalculate_commission здесь нет', () => {
    const post = SRC.slice(SRC.indexOf('export async function POST'));
    const code = post.replace(/\/\*[\s\S]*?\*\//g, '').replace(/^\s*\/\/.*$/gm, '');
    expect(code).not.toMatch(/recalculate_commission/);
  });
});
