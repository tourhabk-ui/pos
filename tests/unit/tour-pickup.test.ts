...
describe('перепись готовности судит по ответу, а не по тексту точки сбора', () => {
  ...
  it('подробности нужны только «встречаемся»: «заберут» и «сам» их не требуют', () => {
    // Владелец 05.09 на вопрос о подвозе: «забирает он туристов сам» — вся
    // фраза. Требовать границы забора значило бы держать туры вне витрин
    // ради поля, которого владелец не просил.
    expect(CENSUS).toMatch(/r\.pickup_type === 'meeting_point' && r\.pickup_details_chars === 0/);
  });
