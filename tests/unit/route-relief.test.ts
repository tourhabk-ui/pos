...
describe('схема внизу экрана показывает трек, если он есть', () => {
  ...
  it('очертания не растянуты: один масштаб по обеим осям', () => {
    // Раньше широта и долгота нормировались каждая по своей оси, а у SVG
    // стоял preserveAspectRatio="none" — трек превращался в ленту и врал о
    // форме маршрута (владелец 09.08: «трек кринж»). Градусы переводятся в
    // метры (долгота короче широты в cos φ раз) и берётся ОДИН масштаб.
    expect(SCREEN).toMatch(/Math\.cos\(\(midLat \* Math\.PI\) \/ 180\)/);
    expect(SCREEN).toMatch(/const scale = Math\.min\(/);
    // На КАРТЕ растяжения нет; на графике профиля оно уместно — это
    // «расстояние × высота», а не местность.
    const mapSvg = SCREEN.slice(SCREEN.indexOf('viewBox="0 0 320 128"'));
    expect(mapSvg.slice(0, 200)).not.toMatch(/preserveAspectRatio="none"/);
  });

  it('карта не притворяется графиком: сетки нет, цвета из токенов', () => {
    expect(SCREEN).not.toMatch(/#0d1b0e|#1a3620/);
    expect(SCREEN).not.toMatch(/stroke="rgba\(74,222,128,0\.06\)"/);
  });

  it('своя позиция на схеме — только по живому фиксу', () => {
    expect(SCREEN).toMatch(/me: coords && figuresLive \? project\(coords\) : null/);
  });
...
describe('константная высота — не рельеф (миграция 894)', () => {
  // Бэкфилл набора (проба 121) записал «0 м» Авачинскому перевалу: у всех
  // точек osm-линии стояло одно и то же число — след заполнения поля, а не
  // прибора. Живой GPS/DEM шумит хотя бы на метры.
  it('идеальная константа на всём треке — данным верить нельзя', () => {
    const r = accumulateRelief(line([120, 120, 120, 120, 120]));
    expect(r.reliable).toBe(false);
  });

  it('равнина с шумом прибора остаётся надёжной, набор честно нулевой', () => {
    const r = accumulateRelief(line([120, 121, 120, 122, 121]));
    expect(r.reliable).toBe(true);
    expect(r.ascentM).toBe(0);
  });
});
