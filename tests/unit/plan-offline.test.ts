...
describe('C-6: клиент — GPX и карта плана', () => {
  it('кнопка «Скачать GPX» ведёт на эндпоинт', () => {
    expect(SHARE_UI).toMatch(/\/api\/trips\/share\/\$\{token\}\/gpx/);
  });

  it('день с координатами ведёт на карту ЭТОЙ страницы, не в чужое приложение', () => {
    // Здесь был deep-link `geo:` — снят 13.09 (владелец: «кнопка навигация до
    // сих пор открывает сторонние сервисы»). geo: — даже не навигатор, а
    // системный выбор приложения, и подпись «Навигатор» обещала программу,
    // которой у получателя ссылки может не быть вовсе.
    //
    // Карта дней уже нарисована на этой же странице, все точки на ней —
    // значит показывать надо её. Офлайн от этого не пострадал: страницу
    // кэширует service worker, а уносимый файл — GPX выше.
    // Ищем ПЕРЕХОД, а не слово: комментарий выше сам объясняет, почему
    // ссылки нет, и запрет на упоминание сделал бы объяснение невозможным.
    expect(SHARE_UI).not.toMatch(/href=\{?[`'"]geo:/);
    expect(SHARE_UI).toContain('href="#trip-map"');
    expect(SHARE_UI).toContain('id="trip-map"');
  });
});
