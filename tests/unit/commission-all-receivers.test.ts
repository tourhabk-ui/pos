...
describe('идемпотентность не потеряна', () => {
  ...
  it('сбой учёта не роняет платёж', () => {
    // Здесь стояло `toMatch(/catch\s*\{/)` — проверка НАПИСАНИЯ, а не смысла.
    // Она требовала пустой формы `catch {` и тем самым запрещала назвать
    // причину: `catch (err)` её ронял. А молчание тут стоило дорого — таблицы
    // `operator_commissions` на проде не существовало, каждая вставка падала,
    // и пустой catch превращал это в «продаж нет» (перепись 22.08).
    //
    // Правило же другое: отказ учёта не должен прерывать платёж. Значит из
    // catch не летит исключение — это и проверяем.
    const tail = LIB.slice(LIB.indexOf('} catch'));
    expect(tail, 'учёт комиссии должен ловить свои ошибки').not.toBe('');
    expect(tail, 'исключение из catch повторило бы весь вебхук').not.toMatch(/\bthrow\b/);
  });
