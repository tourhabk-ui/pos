...
describe('mchs_zones', () => {
  ...
  it('neither volcano, nor district, nor «по краю» → [] (17.09), never a silent avachinsky', () => {
    ...
    // которой в карте нет. Смысл сторожа прежний: незнание — пустота.
    expect(mchs_zones('При достижении уровней неблагоприятного явления на реке Начилова подтоплений не прогнозируется')).toEqual([]);
