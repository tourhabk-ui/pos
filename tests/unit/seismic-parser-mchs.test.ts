...
import { classifyMchsItem, mchs_zones, fetchMchsFeedXml } from '@/lib/services/seismic-parser';
...
describe('fetchMchsFeedXml (candidate URL fallback — confirmed live 2026-07-02: only .../operativnaya-informaciya/rss actually serves RSS)', () => {
  afterEach(() => vi.restoreAllMocks());

  it('uses the first candidate when it returns valid RSS', async () => {
  ...
  it('falls back to the next candidate on a network error', async () => {
    const fetchSpy = vi.spyOn(global, 'fetch')
      .mockRejectedValueOnce(new Error('network down'))
      .mockResolvedValueOnce(new Response('<rss><channel><item><title>ok</title></item></channel></rss>', { status: 200 }));
    const xml = await fetchMchsFeedXml();
    expect(xml).toContain('<item>');
    expect(fetchSpy).toHaveBeenCalledTimes(2);
  });

  it('falls back when a candidate returns HTTP ok but not RSS (e.g. an HTML error page)', async () => {
    const fetchSpy = vi.spyOn(global, 'fetch')
      .mockResolvedValueOnce(new Response('<html><body>404 not found</body></html>', { status: 200 }))
      .mockResolvedValueOnce(new Response('<rss><channel><item><title>ok</title></item></channel></rss>', { status: 200 }));
    const xml = await fetchMchsFeedXml();
    expect(xml).toContain('<item>');
    expect(fetchSpy).toHaveBeenCalledTimes(2);
  ...
  it('falls back when a candidate returns a non-ok HTTP status', async () => {
    const fetchSpy = vi.spyOn(global, 'fetch')
  ...
  it('throws when none of the candidates return valid RSS', async () => {
    vi.spyOn(global, 'fetch').mockResolvedValue(new Response('<html>error</html>', { status: 200 }));
    await expect(fetchMchsFeedXml()).rejects.toThrow(/none of/);
  });
});
