...
describe('mchs_zones', () => {
  ...
  it('neither volcano, nor district, nor «по краю» → [] (17.09), never a silent avachinsky', () => {
    ...
    // 25.09 Большая Воровская внесена в карту с источником (УГМС 13.09: «в
    // районе села Соболево») — пример «места нет в карте» перенесён на реку,
    // которой в карте нет (Начилова и Большая Быстрая внесены следом, словом
    // владельца; пример — просто «Быстрая», тёзка у Эссо). Смысл сторожа
    // прежний: незнание — пустота.
    expect(mchs_zones('При достижении уровней неблагоприятного явления на реке Быстрой подтоплений не прогнозируется')).toEqual([]);
    expect(mchs_zones('При достижении уровней неблагоприятного явления на реке Большой Воровской подтоплений не прогнозируется')).toEqual(['western']);
  });
});
