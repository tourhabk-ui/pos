...
describe('mchs_zones', () => {
  ...
  it('neither volcano, nor district, nor «по краю» → [] (17.09), never a silent avachinsky', () => {
    ...
    // которой в карте нет (Начилова внесена следом, словом владельца; пример
    // — Большая Быстрая). Смысл сторожа прежний: незнание — пустота.
    expect(mchs_zones('При достижении уровней неблагоприятного явления на реке Большая Быстрая подтоплений не прогнозируется')).toEqual([]);
