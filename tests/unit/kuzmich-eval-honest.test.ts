...
describe('судья видит то, чем обоснован ответ', () => {
  ...
  it('в контекст идут туры, поиск и выводы инструментов', () => {
    ...
    // Ответ проходит через guard неподтверждённого километража (#1883,
    // 14.09) — но КОНТЕКСТ уходит судье целиком и неизменным: судья обязан
    // видеть то, чем обоснован ответ, а не то, что мы к нему приписали.
    expect(body).toMatch(/withDistanceCaveat\(cleanAIResponse\(raw\.trim\(\)\), context\)/);
    expect(body).toMatch(/return \{ answer: guarded\.text, context \}/);
  });
...
describe('обрезка контекста не выдаётся за выдумку Кузьмича', () => {
  it('потолок контекста больше измеренного контекста, а не выдуман', () => {
    // Замер прогона 11 (11.09): контекст 25 317 - 28 487 символов у всех
    // тридцати вопросов. Потолок ниже этого означает, что судья постоянно
    // видит половину материала и штрафует Кузьмича за нашу обрезку.
    const limit = num(EVAL, /const JUDGE_CONTEXT_LIMIT = ([\d_]+)/, 'JUDGE_CONTEXT_LIMIT');
    const MEASURED_MAX = 28_487;
    expect(limit).toBeGreaterThan(MEASURED_MAX);
  });
