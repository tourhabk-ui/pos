 ...
 *
 * Сейчас это работает — кнопка выключена, пока галочка не стоит. Но связь
 * держится `disabled`, а не типом: снимет кто-нибудь `disabled` при правке
 * вёрстки, и согласие уедет без галочки, а сервер этого не отличит —
