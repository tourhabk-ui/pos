...
describe('разведданные не судятся как дефекты кода', () => {
  ...
  it('названы числом и списком, а не выброшены молча', () => {
    // С 13.09 разведданные судятся своим вопросом (задание владельца), и в
    // отчёт идёт вердикт с причиной — но само требование прежнее: позиция не
    // исчезает молча.
    const md = renderReport([], undefined, [
      { id: '1', title: 'Внедрить дашборд операторам', verdict: 'internal', reason: 'про нас, не про туриста' },
      { id: '2', title: 'Изучить документацию OpenAI', verdict: 'off', reason: 'не про эту платформу' },
    ]);
    expect(md).toContain('Разведданных: **2**');
