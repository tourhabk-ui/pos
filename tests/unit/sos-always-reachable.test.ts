...
describe('нет второй SOS-кнопки нигде', () => {
  ...
  it('нет сырого tel:112 нигде, кроме EmergencyAction', () => {
    // 24.08: аудит роли туриста нашёл ДЕВЯТЬ мест с собственным
    // `<a href="tel:112">`, ни одно не поймано проверками выше — они ищут
    // подпись «SOS», а эти кнопки называются «112», «СОС» (кириллица) или
    // вообще без слова SOS рядом. Прежний урок про /safety/sos тот же:
    // судить по НАЗНАЧЕНИЮ элемента, а не по конкретной форме разметки.
    // 112 — универсальный номер, на который ведёт EmergencyAction; любая
    // ДРУГАЯ сырая ссылка на tel:112 — вторая реализация того же действия,
    // и рано или поздно разойдётся с ней офлайн-веткой (#887).
    const offenders = [...SOURCES.entries()]
      .filter(([path]) => path !== ACTION)
      .filter(([, src]) => /href=["']tel:112["']/.test(src))
      .map(([path]) => path);
    expect(offenders, 'сырой tel:112 вне EmergencyAction — вторая копия SOS')
      .toEqual([]);
  });
});
