...
describe('провал миграции переживает деплой', () => {
  ...
  it('запись стирается, когда миграция прошла', () => {
    ...
    // Раньше чисток было две: после успеха и в файловой ветке «уже существует».
    // Вторая ветка удалена вместе с самим глотком (находка 14.08): конфликт
    // идемпотентности теперь пропускает выражение под SAVEPOINT, и файл
    // завершается обычным успехом — чистка одна, на пути успеха.
    const deletes = SRC.match(/DELETE FROM _migration_failures WHERE name = \$1/g) ?? [];
    expect(deletes.length, 'после успеха запись о провале должна стираться').toBeGreaterThanOrEqual(1);
  });
});
