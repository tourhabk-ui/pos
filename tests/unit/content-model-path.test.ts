...
describe('генераторы контента подключены к качественному пути', () => {
  ...
  it('судья фактгейта — на качественном пути, как и сам выпуск', () => {
    // ПЕРЕСМОТРЕНО 23.08. Прежде здесь стояло обратное правило: «фактчек
    // остаётся на быстрой ветке — там скорость уместна», и рассуждение
    // выглядело верным. unsupportedClaims возвращает JSON, то есть задача
    // структурная, а не текст для людей; скорость казалась безобидной.
    //
    // Перепись прогонов на проде показала цену этого рассуждения: разведчик
    // молчал 22 дня, и шесть прогонов подряд с 18 по 22.08 отказывали кодом
    // judge_unparseable — «ответила прозой вместо JSON». Быстрая ветка это
    // ГОНКА дешёвых провайдеров, побеждает самый быстрый; формат просили
    // через response_format, но поддерживает его не всякая нога гонки.
    // Структурность задачи как раз и требует провайдера, который держит
    // формат, — а гонка его не гарантирует.
    //
    // Гейт публикации не место для скорости: важно не как быстро ответят, а
    // разберут ли ответ. Порядок теперь тот же, что у выпуска: DeepSeek,
    // затем Qwen.
    ...
    const factCheck = read('lib/agents/fact-check.ts')
      .replace(/\/\*[\s\S]*?\*\//g, '')
      .replace(/(^|[^:])\/\/[^\n]*/g, '$1');
    expect(factCheck, 'судья вернулся на быструю ветку').not.toMatch(/callAIFast/);
    expect(factCheck, 'судья ушёл с качественного пути').toMatch(/callAIQualityOrNull\(/);
