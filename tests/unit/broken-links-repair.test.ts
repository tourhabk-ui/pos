...
describe('уборка не может случиться заодно', () => {
  ...
  it('ждёт КОНКРЕТНЫЙ номер, а не «эндпоинт существует»', () => {
    /**
     * Прогон 10:31 отработал старым кодом при живой проверке: защиту тёзок
     * смержили, номер версии не подняли, и условие «версия не ноль» приняло
     * прежнюю сборку — «Вулкан Вилючинский» снова оказался в списке на
     * снятие.
     *
     * Маркер, который не меняется вместе с поведением, доказывает только то,
     * что маршрут существует.
     */
    expect(WORKFLOW).toMatch(/expect_v/);
    expect(WORKFLOW).toMatch(/\[ "\$V" -ge "\$WANT" \]/);
    expect(WORKFLOW).not.toMatch(/\[ "\$V" != "0" \]/);
    expect(TRIGGER).toMatch(/"expect_v"/);
  });
