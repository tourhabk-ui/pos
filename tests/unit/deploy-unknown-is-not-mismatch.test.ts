...
describe('исход «не доехало» — строгость сохранена', () => {
  ...
  it('образ новее пуша меньше чем на время сборки — «не знаю», а не «доехало» (#1762)', () => {
    // 10.09: мерж 00:02:34, built_at 00:06:44 — четыре минуты, за которые
    // ничего не собирается; это образ предыдущего коммита, законченный после
    // нашего пуша. Свидетель времени здесь молчит, а не подтверждает.
    const r = verdict({
      SERVED_OK: '', SERVED: 'unknown', REASON: 'no_git_head',
      COMMIT: livingSha, EXPECTED_SHA: expectedSha,
      BUILT_EPOCH: '1757240250', NEED_EPOCH: '1757240000',
    });
    expect(r.code).toBe(0);
    expect(r.out).toMatch(/::warning::НЕ СМОГЛИ ПОДТВЕРДИТЬ/);
    expect(r.out).toMatch(/ПРЕДЫДУЩЕГО коммита/);
    expect(r.out).not.toMatch(/::error::/);
    expect(r.out).not.toMatch(/Деплой подтверждён/);
  });
