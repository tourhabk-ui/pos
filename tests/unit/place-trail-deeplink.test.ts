 ...
 * перехода к месту нет»). Дорогу до начала тропы считает свой граф Камчатки
 * (PlaceOwnRoute) — дальше начинается пеший путь, которого у дорожного нет
 * вовсе. Раньше кнопки в это «дальше» с карточки места не было.
 *
...
describe('карточка места несёт деплинк на пеший путь', () => {
  ...
  it('живёт рядом со своим автопутём — тем же разделом карточки', () => {
    // До 13.09 соседом был ряд марок чужих навигаторов (NavigateTo, car):
    // они довозили до начала тропы, а эта ссылка вела дальше. Марки сняты
    // (владелец: «кнопка навигация до сих пор открывает сторонние сервисы»),
    // дорогу считает свой граф — PlaceOwnRoute. Соседство осталось тем же по
    // смыслу: сперва дорога, следом тропа.
    const carAt = PLACE.indexOf('<PlaceOwnRoute lat={place.lat}');
    const linkAt = PLACE.indexOf('mode=trail&q=');
    expect(carAt).toBeGreaterThan(0);
    expect(linkAt).toBeGreaterThan(carAt);
  });
});
