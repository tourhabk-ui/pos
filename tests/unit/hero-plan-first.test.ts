...
describe('план — первым действием', () => {
  it('заголовок героя — посыл, а не topTitle из ленты', () => {
    ...
    // topTitle не может быть содержимым заголовка: он живёт только в alertLine.
    // С 25.09 (П8, #45) заголовок героя — h1, единственный в десктопном
    // дереве; проверка распространена на оба уровня.
    expect(SRC).not.toMatch(/<h[12][^>]*>[^<]*\{headline\}/);
  });
