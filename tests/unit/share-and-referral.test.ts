...
describe('реализация одна', () => {
  it('navigator.share живёт только в lib/share.ts', () => {
    ...
    const candidates = execFileSync(
      'git',
      [
        // `-c color.ui=false` — не косметика, а условие воспроизводимости.
        // У разработчика с `color.ui=always` git grep красит ИМЕНА ФАЙЛОВ
        // ANSI-последовательностями, и путь перестаёт совпадать со строкой
        // 'lib/share.ts': сторож падает на чистом коде, у автора правки, и
        // только у него. На CI цвета нет, поэтому такой отказ невоспроизводим
        // там, где его стали бы разбирать.
        //
        // Сторож обязан судить о КОДЕ. Всё, что делает его ответ зависимым от
        // настроек чужой машины, — дефект самого сторожа.
        '-c', 'color.ui=false',
