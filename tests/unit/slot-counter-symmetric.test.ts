...
describe('оба рабочих пути отмены возвращают места', () => {
  ...
  it('турист: отмена своей брони — атомарный переход, внутри транзакции', () => {
    ...
    // До 11.09 (#1813, решение владельца по сумме возврата) переход отдавал
    // булево `false`. Теперь транзакция возвращает СУММУ возврата (или
    // `null`, если брони не было или платежа не нашлось) — сигнатура шире,
    // не уже: `return null` при неудачном переходе, а не булево.
    expect(CODE).toMatch(/if \(upd\.rowCount === 0\) return null;/);
  });
});
...
describe('ответ об отмене не выдумывает решение о возврате', () => {
  ...
  it('нарисованного «возврат 0 рублей» по-прежнему нет', () => {
    // Тот дефект — ответ УТВЕРЖДАЛ отказ, хотя решения никто не принимал, —
    // не должен вернуться ни в какой форме, включая новую.
    expect(CODE).not.toMatch(/refund: \{ amount: 0, reason: '' \}/);
  });

  it('решение владельца 11.09 — 100%, и сумма реальная, не догадка', () => {
    // «Пока 100%»: временный `refund: null` (честное «не знаю») исполнил
    // свою роль и заменён РЕШЕНИЕМ, а не отменён без замены. Сумма — из
    // tour_payments.retail_amount самой брони, не константа и не с клиента.
    expect(CODE).toMatch(/percent:\s*100/);
    expect(CODE).toMatch(/retail_amount/);
    // Оплаты не было — возврат неприменим, а НЕ ноль: ноль читался бы как
    // отказ вернуть то, что было оплачено.
    expect(CODE).toMatch(/retailAmount\s*!==\s*null/);
  });
