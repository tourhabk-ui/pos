...
describe('основной дайджест защищён так же, как пост в AI-канал', () => {
  ...
  // Якорь — вызов отправки основного выпуска; с 04.09 у него второй аргумент
  // (сток причины отказа Telegram), поэтому скобка не закрывается сразу.
  const body = SRC.match(/Все разделы пусты[\s\S]*?const sent = await tgSend\(digest\b/)?.[0] ?? '';
...
describe('даты не выдумываются (03.09)', () => {
  // Выпуск 16:02 UTC удержан судьёй: «опубликовано 3 сентября 2026» — даты
  // в источнике не было. Молчание второй прогон подряд из трёх допустимых.
