...
describe('server.json — манифест для официального реестра', () => {
  ...
  it('workflow публикации существует, входит ключом домена и краснеет, если реестр нас не видит', () => {
    ...
    /**
     * Поиск реестра отстаёт от записи: run 10 (17.09) опубликовал
     * ru.vedarai/mcp в 10:45:36.69, спросил поиск в 10:45:36.94 — пусто;
     * через полчаса запись была на месте. Проверка переспрашивает с паузой,
     * и красный — только когда не увидели ни разу за отведённое время.
     */
    expect(wf).toMatch(/for attempt in 1 2 3 4 5 6 7 8; do[\s\S]*?sleep 15[\s\S]*?done/);
    expect(wf).toMatch(/за две минуты после публикации/);
    /**
