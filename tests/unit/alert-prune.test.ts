...
describe('сводка из нескольких предметов судится по заголовку', () => {
  // Дословно с прода: перепись /api/cron/alerts-census, прогон 1 (07.09).
  // Целая суточная сводка МЧС уложена в ОДНУ запись: заголовок — отчёт о
  // работе службы, ниже в теле ещё три предмета, тип записи road_closure.
  // На первых 300 символах страж отвечал `service_statistics`, на полном
  // теле — `null`: отбраковку снимала фраза из хвоста, про другое.
  const TITLE = 'К тушению техногенных пожаров сотрудники МЧС России привлекались один раз';
  const BODY = [
    'К тушению техногенных пожаров сотрудники МЧС России привлекались один раз.',
    'Горел мусор на открытой территории.',
    'К ликвидации последствий ДТП сотрудники МЧС России не привлекались.',
    'За сутки сейсмособытий и пепловых выбросов не зарегистрировано.',
    'Продолжается активность вулканов Безымянного, Мутновского, Шивелуча.',
    'Соблюдайте правила пожарной безопасности, проезд по участку ограничен.',
  ].join('\n\n');

  it('заголовок-отчёт не спасается посторонней фразой из тела', () => {
    // Это и есть починка: раньше судилась только склейка, и «соблюдайте» с
    // «ограничен» из последней строки отменяли отбраковку первой.
    expect(rejectedGenre(TITLE, BODY)).toBe('service_statistics');
  });

  it('приговор по одному заголовку тот же', () => {
    expect(rejectedGenre(TITLE)).toBe('service_statistics');
  });

  it('настоящее предупреждение с телом остаётся', () => {
    expect(rejectedGenre(
      'Штормовое предупреждение: ветер до 30 м/с',
      'Ожидается усиление ветра. Тургруппам воздержаться от выхода на маршруты.',
    )).toBeNull();
  });

  it('жанр в одном только теле по-прежнему ловится', () => {
    // Однострочная запись, где глагол жанра живёт в теле: склейка судит как
    // раньше, и эта ветка не должна была пропасть вместе с починкой.
    expect(rejectedGenre(
      'Сводка за сутки',
      'Совершено 12 выездов подразделений, зарегистрировано 3 пожара.',
    )).toBe('service_statistics');
  });
});
