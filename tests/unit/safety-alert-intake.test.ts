...
describe('отказ чтения предупреждений не выдаётся за «всё спокойно»', () => {
  it('пустой catch в загрузчике планировщика не вернулся', () => {
    ...
    // `\n` после `[^\n]*` обязателен намеренно. Прежняя форма
    // `(\/\/[^\n]*\s*)*` неоднозначна: хвостовые пробелы строки может съесть
    // и `[^\n]*`, и `\s*`, и на каждый комментарий приходится по три
    // разбиения — 3^n. Замерено на этой самой регулярке: 12 повторов — 4 мс,
    // 16 — 310 мс, 20 — 25 секунд. Прогон тестов вставал бы намертво от
    // двух десятков комментариев подряд после catch (js/redos).
