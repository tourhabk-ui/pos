...
describe('release gate подключён', () => {
  ...
  it('скрипт ходит в живой чат и падает на нарушениях', () => {
    ...
    // Молча зеленеть при неотвеченном прогоне нельзя. Проверяется СВОЙСТВО,
    // а не буква: 06.09 набор кейсов расширился законными вопросами, и
    // порог стал считаться от общего числа. Прежняя проверка держалась за
    // литерал `GOLDEN_CASES.length / 2` и покраснела на верной правке —
    // та же грабля, что у сторожа заглушки водопада.
    expect(s).toMatch(/results\.length\s*<\s*\w+\.length\s*\/\s*2/);
    // Заглушку водопада прогон обязан отличать от ответа модели: иначе
    // молчание провайдеров объявляется регрессией безопасности.
    expect(s).toContain('classifyAnswer(');
    // Обе ошибки, а не одна: опасный совет и отказ на законном вопросе.
    expect(s).toContain('BENIGN_CASES');
    expect(s).toContain('checkBenign(');
  });
