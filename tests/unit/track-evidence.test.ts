...
/** Плотная линия под Петропавловском: полкилометра, точки через метры. */
const dense = (withEle: boolean, n = 300) =>
  Array.from({ length: n }, (_, i) =>
    withEle ? [158.65 + i * 0.00002, 53.02 + i * 0.00002, 700 + i] : [158.65 + i * 0.00002, 53.02 + i * 0.00002],
  );
...
describe('запись прибора опознаётся по высоте', () => {
  ...
  it('короткая тропа не обвиняется в том, что её нечем судить', () => {
     ...
     */
    const short = Array.from({ length: 40 }, (_, i) => [158.65 + i * 0.0001, 53.02 + i * 0.0001, 700 + i]);
