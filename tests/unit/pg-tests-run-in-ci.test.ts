...
describe('pg-тесты доходят до CI', () => {
   ...
   *
   * Двенадцатый — `guide-team` (25.09, кабинет гида, пакет B). Весь кабинет
   * гида отвечал 500 при зелёных юнитах: uuid = bigint в каждом соединении
   * расписания и туров. Связка «оператор — гид — бронь» (миграции 1018/1019)
   * — это правила доступа к ПД туриста, записанные в SQL, и доказать их может
   * только сервер. Своя база `guide_team_test`, в public не сидит. Порог
   * поднят ЯВНО: замер — файл целиком, 10 тестов, ~6 с вместе с подъёмом базы.
   *
   * Тринадцатый — `agent-money` (26.09, кабинет агента, пакет B). Деньги
   * агента — начисление с оплаченных неотменённых продаж, заявка на выплату
   * и отметка администратора — держатся уникальными индексами и CHECK
   * миграций 1023/1024; мок ответил бы на любой SQL. Своя база
   * `agent_money_test`. Порог поднят ЯВНО: замер — 9 тестов, ~10 с вместе с
   * подъёмом базы.
   */
  it('число pg-файлов не переросло размен «последовательно вместо изоляции»', () => {
    const SEQUENTIAL_BUDGET = 13;
