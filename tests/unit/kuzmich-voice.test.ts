...
describe('голос — одна константа на все промпты Кузьмича', () => {
  it('каждый канальный промпт Кузьмича несёт KUZMICH_CHANNEL_VOICE', () => {
    ...
    // 05.09 два промпта ушли: место и тур собираются из данных без модели
    // (решение владельца после поста про озеро Зелёное). Остались совет,
    // сезон (ручная команда) и «друг платформы».
    expect(total, 'промпты Кузьмича не найдены — сторож ослеп').toBeGreaterThanOrEqual(3);
