...
describe('ноги гонки перестали глотать причину', () => {
  ...
  it('проба идёт живым путём судьи, а не только путём чата', () => {
    // Замер на проде 23.08: строка deepseek зелёная (381 мс), а судья при этом
    // сообщал «не ответил ни один провайдер». Противоречия нет — проба брала
    // модель назначения chat без формата, судья идёт content + json_object.
    // Проверка, не ходящая живым путём, зелена ровно настолько же, насколько
    // бесполезна.
    expect(providers).toMatch(/probeDetailed\('deepseek:content'/);
    expect(providers).toMatch(/resolveContentModel\('deepseek'\)[\s\S]{0,400}response_format/);
  });
