...
describe('улика о линии старше ярлыка о поставщике', () => {
  ...
  it('улика считается по СЫРОЙ геометрии, а не по прореженной линии', () => {
    ...
    //
    // Сборка вердикта 21.09 вынесена из роута в `card-navigability`: у неё
    // стало два читателя (карточка и объяснение решения), и второй обязан
    // пересказывать первый, а не считать свой. Сторож переехал за правилом —
    // держать место реализации значило бы краснеть на каждом переносе и
    // молчать о самой подмене данных.
    const SRC = readFileSync(join(process.cwd(), 'lib/routes/card-navigability.ts'), 'utf-8');
    expect(SRC).toMatch(/trackEvidence\(i\.geometry\)/);
    // Прореженная линия рядом — именно её сюда подставили бы по невниманию.
    expect(SRC).not.toMatch(/trackEvidence\(\s*track/);
  });
});
...
describe('черта живёт в одном месте, поверхности её спрашивают', () => {
  ...
  it('вердикт считается на сервере — там есть и линия, и точки', () => {
    ...
    expect(API).toMatch(/routeCardNavigability\(/);
    expect(API).toMatch(/navigability:/);
  });

  it('путевые точки доходят до черты', () => {
    // С 01.09 не доходили: фильтр спрашивал `w.lat`, а запрос отдаёт
    // `place_lat`, и вердикт «линию не с чем сверить» получал КАЖДЫЙ маршрут
    // (разбор — route-card-waypoints-reach-verdict).
    expect(API).toContain('.filter(hasCoords)');
    expect(API).not.toMatch(/w\.lat\s*!=\s*null/);
  });
