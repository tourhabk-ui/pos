...
describe('экран пользуется этим, а не рисует уверенность', () => {
  it('неподтверждённая стрелка не рисуется вовсе', () => {
    // Прибор переехал в components/field/FieldCompass (приборный вид по
    // макетам FCN). Правило ужесточено 21.08: раньше неподтверждённая
    // стрелка гасла прозрачностью, но на солнце прозрачность не читается —
    // серая стрелка в абсолютном угле выглядела рабочей. Теперь угол стрелки
    // существует только при подтверждённом азимуте; без него азимут говорит
    // число (см. field-compass-honesty.test.ts).
    const C = readFileSync(join(process.cwd(), 'components/field/FieldCompass.tsx'), 'utf-8');
    expect(C).toMatch(/const trusted = state === 'ok'/);
    expect(C).toMatch(/needleAngle = trusted && targetBearing !== null/);
  });
...
describe('экран соответствует моменту, а не отчитывается о датчиках', () => {
  ...
  it('без маршрута — выбор цели вместо приборной панели с нулями (destination-first, 27.08)', () => {
    // Правило то же, формулировка сменилась: раньше пустое состояние было
    // «Маршрут не выбран» + одна кнопка «Выбрать маршрут», открывающая
    // поиск ВНУТРИ модалки. UX-коррекция владельца сделала destination-first
    // первичным: без маршрута экран сразу показывает инструмент поиска цели
    // («Куда хотите пойти?»), а не приборы без сигнала и карточки с нулями.
    const condAt = SCREEN.indexOf('{!hasRoute && !isLoadingRoute ? (');
    expect(condAt).toBeGreaterThan(0);
    const emptyState = SCREEN.slice(condAt, SCREEN.indexOf(') : (', condAt));
    expect(emptyState).toMatch(/Куда хотите пойти\?/);
    expect(emptyState).toMatch(/renderDestinationPicker\(\)/);
    // Приборная формулировка ушла из ЭТОГО состояния — не проверяем весь
    // файл: та же фраза законно живёт у пустой полноэкранной карты
    // («карта без трека» — другой контекст, не про выбор маршрута).
    expect(emptyState).not.toMatch(/Маршрут не выбран/);
  });
