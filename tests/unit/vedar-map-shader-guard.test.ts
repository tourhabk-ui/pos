...
describe('VedarMap: сбой кадра — не чёрное поле', () => {
  ...
  it('обе заметки называют зум ЦИФРОЙ — иначе скрин не отличает одну беду от другой', () => {
    // Скрин владельца 04.09 07:42: надпись «масштаб мельче пакета», а рельеф
    ...
    expect(body).toMatch(/вне всех районов реестра \(зум \$\{z\}\)/);
    expect(MAP).toMatch(/const z = zoom\.toFixed\(1\);/);
  });
