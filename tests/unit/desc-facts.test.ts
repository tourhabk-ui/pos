...
/**
 * Судья v5 (21.08): мера принадлежит тому, к чему прицеплена.
 *
 * «Каньон длиной 4 км» — размер предмета, а не длина пути к нему; запись
 * маршрута такому числу не противоречит, и спор тут выдуман.
 */
describe('parseClaimedNumbers — чья мера', () => {
  it('длина каньона не становится длиной маршрута', () => {
    const c = parseClaimedNumbers('Тропа выводит к каньону. Каньон длиной 4 км впечатляет.');
    expect(c.distanceKm).toBeNull();
  });

  it('обратный порядок — «длина каньона 4 км» — тоже про предмет', () => {
    const c = parseClaimedNumbers('Маршрут идет по краю. Длина каньона — 4 км.');
    expect(c.distanceKm).toBeNull();
  });

  it('мера, прицепленная к пути, остается утверждением о пути', () => {
    expect(parseClaimedNumbers('Маршрут длиной 12 км.').distanceKm).toBe(12);
    expect(parseClaimedNumbers('Протяженность тропы — 8 км.').distanceKm).toBe(8);
    expect(parseClaimedNumbers('Протяженность маршрута 15 км.').distanceKm).toBe(15);
  });

  it('после пропущенной меры предмета берется следующая — о пути', () => {
    const c = parseClaimedNumbers('Каньон длиной 4 км. Протяженность маршрута 18 км.');
