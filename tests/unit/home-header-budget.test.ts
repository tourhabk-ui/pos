 ...
 * прежних замера молча мерили fallback): бренд 72.63 + пилюля 146.34 (худший
 * текст) + SOS 72.53 + иконки 88 + зазоры 60 = 439.50px против доступных
 * 320/350/372 при 360/390/412. Шапка перегружена на любой мобильной ширине,
 * поэтому одного `flex:none` недостаточно — нужен и уход бренда (порог 427
 * по «Опасности» 93.77), и страховка flex-wrap для длинной пилюли.
 */
...
describe('шапка не может переполниться по горизонтали', () => {
  ...
  it('бренд уходит на узких ширинах — он единственный уступает', () => {
    ...
    // с брендом состояние «Опасность» требует 426.93px, то есть viewport 427+.
