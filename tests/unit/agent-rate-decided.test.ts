...
const ADMIN    = readFileSync(join(ROOT, 'app/api/admin/agent-referral/rate/route.ts'), 'utf-8');
...
describe('агент не назначает себе ставку', () => {
  ...
  it('присланная ставка ОТКЛОНЯЕТСЯ, а не выбрасывается молча', () => {
    ...
    expect(CABINET).toMatch(/назначает платформа, а не агент/);
...
describe('ставку назначает администратор, с автором и основанием', () => {
  ...
  it('автор и время пишутся вместе со ставкой', () => {
    expect(ADMIN).toMatch(/rate_set_by\s*=\s*\$3/);
    expect(ADMIN).toMatch(/rate_set_at\s*=\s*NOW\(\)/);
    expect(ADMIN).toMatch(/rate_reason\s*=\s*\$4/);
    expect(MIGRATION).toMatch(/rate_set_by/);
    expect(MIGRATION).toMatch(/rate_set_at/);
    expect(MIGRATION).toMatch(/rate_reason/);
  });
  ...
  it('ноль разрешён как решение, и это не то же, что «не назначена»', () => {
    ...
    expect(ADMIN).toMatch(/rate:\s*z\.number\(\)\.min\(0\)/);
  });

  it('ссылки с таким кодом нет — отказ, а не тихий ноль строк', () => {
    expect(ADMIN).toMatch(/rows\.length === 0/);
    expect(ADMIN).toMatch(/status:\s*404/);
  });

  it('роут не притворяется выплатой', () => {
    expect(ADMIN).toMatch(/путь выплаты не подключён/);
  });
...
describe('пустая ставка нигде не становится нулём', () => {
  it('на сервере итог считается только по ссылкам со ставкой', () => {
    expect(CABINET).toMatch(/rows\.filter\(r => r\.commission_rate !== null\)/);
    expect(CABINET).toMatch(/linksWithoutRate/);
  ...
  // Проверка ПОРЯДКОМ, а не запретом подстроки. Первая редакция запрещала
  // `{Number(link.commission_rate)}%` — и краснела на исправленном коде:
  // в нём та же подстрока живёт внутри шаблона `${...}%`, то есть УЖЕ под
  // проверкой на пустоту. Запрет ловил форму, а важен порядок: число
  // считается только после того, как отсутствие отсеяно. Снимут проверку —
  // индекс пропадёт, и сторож покраснеет по делу.
  it('на экране ставка превращается в число только после проверки на пустоту', () => {
    const guard = UI_CODE.indexOf('link.commission_rate === null');
    const used  = UI_CODE.indexOf('Number(link.commission_rate)');
    expect(guard, 'проверки на пустую ставку нет вовсе').toBeGreaterThan(-1);
    expect(used, 'ставка нигде не показывается').toBeGreaterThan(-1);
    expect(guard, 'число считается раньше проверки').toBeLessThan(used);
  ...
  it('заработок превращается в рубли только после проверки на пустоту', () => {
    ...
    const used  = UI_CODE.indexOf('Number(link.earned_total)');
  ...
  it('типы допускают отсутствие — иначе пустоту нечем выразить', () => {
    expect(UI).toMatch(/commission_rate:\s*string \| null/);
    expect(UI).toMatch(/earned_total:\s*string \| null/);
  });
});
