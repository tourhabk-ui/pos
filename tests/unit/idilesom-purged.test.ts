...
describe('к сайту источника больше не ходят', () => {
  ...
  const CLEANUP_ALLOWLIST = new Map<string, string>([
    ...
    ['lib/routes/geometry-precedence.ts',
     'прежний слог линии в LEGACY_SLUGS — та же страховка, что у ' +
     'line-standard, только про ПРАВО ПЕРЕЗАПИСИ: забыв его, правило сочтёт ' +
     'слог неизвестным, а неизвестный не перекрывается никем — и такая линия ' +
     'осталась бы навсегда, даже когда придёт снятый прибором трек'],
    ...
    ['lib/images/origin.ts',
     'называет ПРИЧИНУ, по которой 340 снимков скрыты: их род записан в ' +
     'колонке model как idilesom-*, миграция 941 обезличила подпись источника ' +
     'и метку геометрии, но до родов снимков не дотягивалась. Без имени в ' +
     'условии эти 340 объяснялись бы как «род не в списке» — формально верно ' +
     'и бесполезно тому, кто спрашивает, почему у места нет фото'],
