...
describe('выкладка в хранилище', () => {
  ...
  it('переиспользует бакет фото, а не требует заводить второй', () => {
    // Хранилище одно; карта ложится в папку maps/ рядом с фото. Имена MAP_*
    // остаются на случай отдельного бакета и имеют приоритет.
    expect(wf).toMatch(/secrets\.MAP_S3_BUCKET \|\| secrets\.S3_BUCKET/);
    expect(wf).toMatch(/secrets\.MAP_S3_KEY \|\| secrets\.S3_ACCESS_KEY/);
    expect(wf).toMatch(/s3\.twcstorage\.ru/);
  });

  it('публичный адрес берётся из настройки, а не выводится из эндпоинта', () => {
    // Хранилища раздают файлы то путём, то поддоменом. Угадывание уронило бы
    // проверку на верно настроенном бакете — чинили бы то, что не сломано.
    expect(wf).toMatch(/S3_PUBLIC_BASE/);
    expect(wf).toMatch(/BASE="\$\{S3_PUBLIC_BASE:-/);
  });

  it('проверяется тот же ключ, который выложен', () => {
    // Дата, посчитанная дважды, разъезжается на границе суток: выложили в
    // 23:59, проверяем в 00:00 — и проверка стучится в несуществующий файл.
    expect(wf).toMatch(/echo "\$KEY" > \.map-key/);
    expect(wf).toMatch(/KEY=\$\(cat \.map-key\)/);
  });
