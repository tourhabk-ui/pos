...
describe('«Расходы AI» — две вкладки на одном журнале', () => {
  ...
  it('вкладка берётся из адреса — редирект и старые ссылки открывают нужную', () => {
    // Проверяется НАМЕРЕНИЕ, а не буква: вкладок стало три (09.09 добавлены
    // цены моделей), и сравнение переехало из выражения в переменную. Сторож,
    // прибитый к точной строке, красен на верной правке — а такой краснотой
    // учат не доверять сторожам.
    expect(PAGE).toMatch(/URLSearchParams\(window\.location\.search\)\.get\('tab'\)/);
    expect(PAGE).toMatch(/tabFromLocation\(\): UsageTab[\s\S]{0,300}'kuzmich'/);
