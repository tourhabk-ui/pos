...
describe('пуш при закрытии дороги', () => {
  // 06.09: решение отменено (владелец) — road_closure убран из push. Одно и
  // то же ограничение приходит сразу с нескольких источников (kamgov, Минтур,
  // ВК МЧС) с чуть разным текстом; контент-дедуп на вставке сверяет ДОСЛОВНО
  // и разные формулировки не ловит — турист получал по 5-6 push об одном и
  // том же перекрытии. Данные по-прежнему видны на /safety и в офлайн-пакете
  // (описание выше в файле) — только без push.
  it('road_closure НЕ проходит порог severity — только tsunami_warning особый', () => {
    expect(CRON).toContain("severity >= 2 OR alert_type = 'tsunami_warning'");
    expect(CRON).not.toContain("alert_type IN ('tsunami_warning', 'road_closure')");
  });

  it('у pushCopy остаётся свой текст для road_closure — на будущее и для ручных алертов severity>=2', () => {
