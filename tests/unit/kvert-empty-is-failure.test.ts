...
describe('ноль вулканов = провал прогона', () => {
  ...
  it('успешный прогон несёт счётчик работы в телеметрию', () => {
    ...
    //
    // Проверяется НАЛИЧИЕ счётчика, а не конкретное поле. 14.09 счётчик
    // сменился с `fetched` (разобрано из выпуска KVERT) на `matched`
    // (доведено до карточки места) — вместе с двумя новыми отказами, см.
    // tests/unit/kvert-matched-guard.test.ts. Прежняя редакция была
    // приколочена к форме и покраснела бы на правке, которая её собственный
    // смысл усиливает: сторож холостых прогонов должен видеть доведённую
    // работу, а не факт чтения чужой страницы.
    expect(ROUTE).toMatch(/'success', \{ items: result\.(matched|fetched) \}/);
  });
});
