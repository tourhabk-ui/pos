-- 991: снимок каньона на место машинной картинки — и дубль наконец снят.
--
...
BEGIN
  ...
  IF v_gam_ark IS NULL THEN
    RAISE WARNING '[991] у «Крыльев Гамулов» нет ark_id — снимку ехать некуда, дубль ОСТАЁТСЯ видимым';
    RETURN;
  END IF;
  ...
  IF v_author IS DISTINCT FROM 'Андрей Запорожец' THEN
    RAISE WARNING '[991] снимок НЕ встал на «Крылья Гамулов»: там % (model=%); дубль намеренно оставлен видимым',
      COALESCE('автор «' || v_author || '»', 'строки снимка нет'), COALESCE(v_model, '—');
  END IF;

  IF v_dup_live IS TRUE THEN
    RAISE WARNING '[991] дубль «Каньон на Шивелуче» всё ещё видим — перезалив пакетов карты ничего не исправит, пока это так';
  END IF;
END $$;
