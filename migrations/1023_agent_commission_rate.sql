-- 1023: ставка вознаграждения агента — одна на агента, назначает владелец.
--
...
COMMENT ON COLUMN agent_referral_links.commission_rate IS
  'Историческая ставка ссылки (до 26.09). Деньги её НЕ читают: ставка агента одна — partners.agent_commission_rate (миграция 1023).';
...
UPDATE partners p
   SET agent_commission_rate = x.rate,
       ...
       agent_rate_reason     = 'перенесено со ставки реферальной ссылки (миграция 1023): ' || COALESCE(x.reason, 'основание не записано')
  FROM (
    SELECT agent_id,
