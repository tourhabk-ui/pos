...
/**
 * ФИКСИРОВАННЫЙ СЛОЙ НА КАРТОЧКЕ ОДИН — SOS, и живёт он в `PlaceSOS`
 * (page.tsx), не здесь.
 *
 * До 24.08 в этом файле был свой `<a href="tel:112">СОС</a>` (кириллица —
 * мимо латинского regex сторожа sos-always-reachable.test.ts), и он рисовался
 * ОДНОВРЕМЕННО с `PlaceSOS`: два `fixed bottom-0` бара поверх друг друга,
 * верхний по z-index прятал нижний. Потом здесь остался бар со скачиванием
 * GPX, разведённый с SOS по высоте вручную.
 *
 * 19.09 (направление D, срез 3) ушёл и он — по двум причинам.
 *
 * Первая: слоёв было ТРИ. Липкая шапка действий, фиксированный бар GPX и
 * фиксированный SOS на экране в 844 пикселя съедали верх и низ, оставляя под
 * чтение середину.
 *
 * Вторая важнее. Бар стоял на `bottom: calc(safe-area + 52px)`, и число 52
 * означало «высота PlaceSOS без safe-area»: два фиксированных бара держались
 * друг от друга КОНСТАНТОЙ, списанной с чужого компонента. Наехать друг на
 * друга они уже успели однажды, и любая правка высоты SOS вернула бы это
 * молча — причём закрыт оказался бы SOS, а закрыл бы его бар со скачиванием
 * файла.
 *
 * Сам файл никуда не делся и не стал дальше: GPX теперь тихой ссылкой рядом с
 * пешим путём — в колонке «как добраться», которая на телефоне идёт сразу под
 * шапкой (порядок DOM выбран ровно для этого, решение владельца 07.09).
 * Действий на первом экране стало меньше, а офлайн-файл остался там, где
 * человек его ищет — рядом с вопросом «как я туда попаду».
 *
 * Прежнее разделение труда при этом сохранено: единственное ДЕЙСТВИЕ-кнопка
 * на карточке — «Проложить путь» в `PlaceActionBar` (07.09, владелец: «почему
 * 2 кнопки навигация?»; 13.09 она же перестала звать чужие навигаторы и будит
 * наш расчёт). Всё остальное — ссылки, а не кнопки.
 */
      ...
      */}
      <div className="mx-auto w-full max-w-3xl px-4 lg:max-w-6xl lg:px-6 lg:grid lg:grid-cols-[minmax(0,1fr)_340px] lg:gap-10 lg:items-start">
        ...
        <aside className="lg:col-start-2 lg:row-start-1 lg:sticky lg:top-[120px] pt-3 space-y-3">
          ...
          {/* Офлайн-файл трека — тихой ссылкой рядом с пешим путём, а не
              фиксированным баром внизу экрана (срез 3, см. комментарий выше).
              Оба отвечают на один вопрос «как я туда попаду», и разносить их
              по разным краям экрана было нечем оправдать.
              Это ЕДИНСТВЕННАЯ ссылка на GPX места: вторая копия действия
              расходится поведением (#887). */}
          <a
