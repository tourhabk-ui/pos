...
Солнцезащитный крем SPF 50+ обязателен — на высоте ультрафиолет значительно интенсивнее. Тёплый слой всё равно нужен: вечером температура падает до +5°C даже в июле.`,
  },
  {
    ...
    tag: 'Безопасность',
    date: '2026-05-20',
    content: `На Камчатке покрытие мобильной сети заканчивается примерно в 30 км от Петропавловска. Большинство популярных маршрутов проходит вне зоны 4G. Именно поэтому SOS-функция TourHab работает без интернета.
...
4. **Экстренные номера работают всегда.** 112 и +7-4152-235-362 (МЧС Камчатского края) работают через сотовую сеть — без интернета.
...
Скачайте маршрут офлайн через кнопку "Скачать" на странице маршрута. Это создаст локальную копию треков и безопасных точек.`,
  },
  {
    ...
    tag: 'Операторы',
    date: '2026-05-10',
    content: `На Камчатке несколько десятков туроператоров, но качество сервиса и уровень безопасности у них разный. TourHab добавляет в каталог только верифицированных операторов.
...
TourHab не показывает туры без привязки к конкретному юрлицу или ИП. Если вы видите тур без информации об операторе — это не TourHab.
