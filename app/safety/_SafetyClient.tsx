          ...
          {(live.safety.alerts.length > 0 || live.seismic.events.length > 0
            ...
            <div className="safety">
              ...
              */}
              <div className="src">
                {/*
                  У вулканов ДВЕ отметки, и они отвечают на разные вопросы.

                  Владелец 15.09 увидел здесь «КВЕРТ (вулканы) — обновлено 4 дн
                  назад» и спросил, что это значит. Одна строка покрывала два
                  несовместимых состояния: «КВЕРТ четвёртый день не публикует
                  новых наблюдений» (тишина на вулканах, данные верные) и «наш
                  синк четвёртый день лежит» (данные протухли, цвет на экране
                  может быть любым). Отличить их было нельзя — при том что
                  прямо под строкой стоит «Опасность: Высокая».

                  Тот же класс, что разбор 07.09 в комментарии выше: одна цифра
                  на несколько источников. Поэтому «наблюдение» и «спросили»
                  печатаются раздельно и называются своими словами.
                */}
                КВЕРТ (вулканы): наблюдение {live.volcanoes.updatedAt ? fmtAgo(live.volcanoes.updatedAt) : 'без даты'}
                {' · '}спросили {live.volcanoes.checkedAt ? fmtAgo(live.volcanoes.checkedAt) : 'неизвестно когда'}
                {live.seismic.events.length > 0
