  ...
  try {
    ...
    for (const route of activeRoutes.rows) {
      ...
      if (stepToRun <= 2) {
        ...
        if (route.user_id) {
          const userChat = await query<{ telegram_chat_id: string }>(
            `SELECT telegram_chat_id FROM users WHERE id = $1 AND telegram_chat_id IS NOT NULL`,
            [route.user_id]
          ).catch(() => ({ rows: [] as { telegram_chat_id: string }[] }));
          if (userChat.rows.length > 0) {
            ...
            const text = stepToRun === 1
              ...
              : `Повторное напоминание: маршрут "${route.route_name}" не закрыт. Если всё в порядке, отметь возврат: vedarai.ru/safety/return?id=${route.id}`;
        ...
        const text = [
          `Здравствуйте, ${route.emergency_contact_name}.`,
          ``,
          `${route.leader_name} должен был вернуться ${route.end_date}.`,
          `Прошло ${daysDiff} дн. Возврат не отмечен.`,
          ``,
          `Рекомендуем НЕМЕДЛЕННО обратиться в МЧС:`,
          `• Единый номер: 112 (работает без баланса и SIM, переключит на МЧС/спасателей)`,
          ``,
