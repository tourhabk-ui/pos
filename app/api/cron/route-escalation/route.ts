      ...
      if (route.user_id) {
        ...
        if (userChat.rows.length > 0) {
          ...
          const text = stepToRun === 1
            ? `🔔 Напоминание: ты должен был вернуться с маршрута "${route.route_name}" сегодня (${route.end_date}). Всё в порядке? Нажми "Я вернулся" в приложении: vedar.app/safety/return?id=${route.id}`
            : `⚠️ Повторное напоминание: маршрут "${route.route_name}" не закрыт. Если всё в порядке, отметь возврат: vedar.app/safety/return?id=${route.id}`;
