 ...
 *   2 — починено имя колонки броней (operator_tour_id, а не tour_id)
 *
 * Версия поднята именно ради починки, хотя форма ответа не изменилась.
 * 19.08 прогон после мержа снова вернул 500 со старой ошибкой: правка уехала,
 * но прод ещё не пересобрался, а ворота ждали версию 1 — которую отдавал и
 * СЛОМАННЫЙ контейнер. Ворота, которые пропускают неисправленный код, не
 * ворота: любая правка поведения обязана двигать номер, иначе прогон спросит
 * прежний прибор и покажет прежнюю ошибку как свежую.
 */
export const POPULARITY_VERSION = 2;
