...
export async function GET(request: NextRequest) {
  ...
  const providerIssues: Array<HealthIssue & { reason: string }> = [];
  {
    ...
    // Qwen 08.09 снимался с ТЕКСТОВЫХ путей («qwen не используем») и 24.09
    // возвращён туда решением владельца — второй ногой цикла инструментов
    // Кузьмича и первой фазой scout-innovator. Главным в предупреждении всё
    // равно остаётся зрение: у текста есть следующие ступени, у фото — нет.
    //
    // Зрение (qwen-vl, разбор фото в чате и в загрузке снимков)
  ...
  return NextResponse.json({
    ...
    integrations: {
      ...
      // Подтверждение прав в Яндекс.Вебмастере. Код мета-тег отдаёт всегда
      // (app/layout.tsx, verification.yandex), но ровно тогда, когда есть
      // переменная — иначе Next просто не печатает тег, молча.
      //
      // Разница дорогая: без подтверждения Вебмастер не считает сайт нашим,
      // и «мало трафика» становится неотличимо от «нас не индексируют».
      // Отличить снаружи иначе нечем: по HTML видно только отсутствие тега,
      // а причина — не задано или сломалось — из него не следует.
