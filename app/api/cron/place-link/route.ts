 ...
 *
 * Bearer CRON_SECRET. Body: { pairs: [{place, route}] (1..50), dry_run }.
  ...
  try {
    ...
    for (const r of rows) {
      ...
      plan.push({
        ...
        nameScore: nameMatchScore(r.place_name ?? '', r.route_title ?? ''),
        distanceKm: d,
    ...
    return NextResponse.json({
      ...
      linked: linked.map(p => ({ place: p.placeName, route: p.routeTitle, nameScore: p.nameScore, distanceKm: p.distanceKm })),
    });
  } catch (err) {
