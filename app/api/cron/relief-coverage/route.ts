 ...
 * `idilesom-importer` писал их прямо в `kamchatka_routes.geometry`, причём
 * предпочитал формат С высотой. Значит вопрос решается замером, а не
 ...
 *
 * Прошедшее время с 07.09: сам импортёр вычищен решением владельца вместе со
 * всем скрейпом чужого сайта, а скачанные линии остались и числятся родом
 * `external` (миграция 941). Замер от этого не устарел — он и считает то, что
 * лежит в базе, а не то, что её наполняло.
 *
