  ...
  try {
    ...
    if (adminEmail) {
      emailService.sendEmail({
        ...
        html: `<p>Зарегистрировался новый оператор.</p>
               ...
               <b>Email:</b> ${email}</p>
               <p><a href="https://vedar.app/hub/admin/operators">Очередь заявок →</a></p>`,
      }).catch(() => {});
    }
    ...
    emailService.sendEmail({
      ...
      html: `
        ...
        <ol>
          <li><a href="https://vedar.app/hub/operator">Войдите в личный кабинет</a></li>
        ...
        <p>Вопросы? Пишите: <a href="mailto:${adminEmail ?? 'info@vedar.app'}">${adminEmail ?? 'info@vedar.app'}</a></p>
        <hr>
        <p style="color:#888;font-size:12px">KamchatourHub — vedar.app</p>
      `,
    }).catch(() => {});
