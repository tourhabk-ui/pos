...
export async function POST(request: NextRequest) {
  ...
  const result = await transaction(async client => {
    ...
    // Вызова `recalculate_commission` здесь БОЛЬШЕ НЕТ (разбор 11.09).
    //
    // Прежний комментарий объяснял, почему он внутри транзакции: «иначе при
    // откате выплаты он остался бы посчитанным по платежам, которые так и не
    // выплачены». Довод разумный — и относится не к тому, что делает функция.
    // По имени она читается как «пересчитать комиссию по этим платежам»; на
    // деле она не трогает ни `operator_commissions`, ни `tour_payments`, а
    // ПЕРЕЗАПИСЫВАЕТ `partners.commission_current` — ставку, по которой
    // считается каждое будущее начисление — по лестнице за объём завершённых
    // броней. При ставке 10% (решение владельца 04.08, миграция 811) оператор
    // с десятью завершёнными бронями молча уводил платформу на 7%, с
    // пятьюдесятью — на 5%.
    //
    // Ставка — решение владельца, а не следствие нажатия «выплатить».
    // Сторож: `tests/unit/commission-rate-decided.test.ts`.
