        ...
        if (partnerId) {
          ...
          const sdkResult = await runSDKAgent({
            ...
            systemPrompt: systemPrompt + `\n\nТы — AI-ассистент оператора туров на Камчатке. ` +
              ...
              // Принцип Volcano OS (§8: принципы, не перечни кейсов): мутации
              // проходят через ядро, и отказ ядра — решение, а не сбой.
              `Если инструмент ответил отказом — «требуется одобрение человека», ` +
              `«запрещено», «конфликт» — передай отказ оператору дословно и не ` +
              `пытайся добиться того же действия другим инструментом или повтором. ` +
