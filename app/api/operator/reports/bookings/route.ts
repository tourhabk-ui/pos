...
export async function GET(request: NextRequest) {
  try {
    ...
    const leadTimeResult = await query<OpLeadTimeRow>(
      `SELECT
        AVG(EXTRACT(DAY FROM (b.booking_date - b.created_at::DATE))) as avg_lead_time_days,
        MIN(EXTRACT(DAY FROM (b.booking_date - b.created_at::DATE))) as min_lead_time_days,
        MAX(EXTRACT(DAY FROM (b.booking_date - b.created_at::DATE))) as max_lead_time_days
      FROM operator_bookings b
      JOIN operator_tours t ON b.tour_id = t.id
      WHERE t.operator_id = $1
        AND b.created_at >= $2
    ...
    const guestsResult = await query<OpGuestsDistributionRow>(
      `SELECT
        CASE
          WHEN b.participants = 1 THEN '1'
          WHEN b.participants BETWEEN 2 AND 4 THEN '2-4'
          WHEN b.participants BETWEEN 5 AND 10 THEN '5-10'
          ELSE '10+'
        END as group_size,
        COUNT(*) as count
      FROM operator_bookings b
