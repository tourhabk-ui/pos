  ...
  try {
    ...
    const financeQuery = `
      SELECT
        ...
        COALESCE(SUM(CASE WHEN b.booking_status IN ('confirmed', 'completed') AND b.payment_status = 'pending' THEN b.total_price ELSE 0 END), 0) as pending_payouts,
        COALESCE(SUM(CASE WHEN b.booking_status = 'completed' AND b.payment_status = 'paid' THEN b.total_price ELSE 0 END), 0) as completed_payouts,
        COALESCE(SUM(CASE WHEN b.booking_status IN ('confirmed', 'completed') THEN b.total_price * 0.15 ELSE 0 END), 0) as commission,
        COALESCE(SUM(CASE WHEN b.booking_status IN ('confirmed', 'completed') THEN b.total_price * 0.85 ELSE 0 END), 0) as net_income
