...
function OnTrailTab({ mapPackBaseUrl, topInset }: { mapPackBaseUrl: string | null; topInset: number }) {
    ...
    try {
      ...
      if (q && q.trim()) {
        ...
        if (params.get('auto') === '1') {
          cameFromPlaceRef.current = true;
          // ВЫБОР ЦЕЛИ ОТКРЫВАЕТСЯ ЯВНО (правка 15.09).
          //
          ...
          //
          // Причина: поиск по имени работает, только пока виден выбор цели
          // (`pickerVisible = showRouteModal || нет активного маршрута`). Если
          // у человека уже был сохранён активный маршрут, выбор не показан,
          // поиск не запускается, `q` из адреса остаётся в невидимом поле — и
          // экран честно навигирует по ПРЕЖНЕМУ маршруту. Снаружи это
          // выглядит как «ссылка ведёт не туда», и в поле это опаснее
          // неработающей ссылки: компас уверенно указывает на чужую точку.
          //
          // Сохранённый маршрут при этом НЕ стирается: человек мог быть на
          // нём прямо сейчас. Открывается выбор поверх — и решение остаётся
          // за человеком, как того и требовало решение владельца 30.08
          // (автовыбор доводит до цели, но не отменяет «Изменить»).
  ...
  const [tileDl, setTileDl] = useState<{ done: number; total: number; unit: 'МБ' | 'файлов' } | null>(null);
  ...
  const [mapPlan, setMapPlan] = useState<{
    ...
    coverage: 'corridor' | 'bbox' | 'packs'; bufferKm: number | null; urls: string[];
    /** Файлы своих пакетов (24.09) — то, что кнопка кладёт в телефон. */
    files: PackFile[];
    /** Сколько файлов сервер не назвал по весу: МБ на кнопке — нижняя граница. */
    unknownSize: number;
  } | null>(null);
  /**
  ...
  const fieldActions = useMemo<FieldAction[]>(() => {
    ...
    if (hasRoute && mapPlan && !savedMap) {
      ...
      list.push({
        ...
        busy: downloading,
        hint: downloading
          ? `${tileDl!.done}/${tileDl!.total} ${tileDl!.unit}`
    ...
    <div className="relative min-h-[calc(100vh-56px)]" style={{ color: 'var(--text-primary)' }}>
      ...
      {(hasRoute || (status && status.tone !== 'warn')) && (
        <div className="fx-glass mx-3 mt-3 rounded-2xl overflow-hidden">
          ...
          {/* Почему подложка такая, какая есть.
              01.09: своя карта не включалась, и три часа ушло на переписку с
              владельцем — «Паратунка? старый бандл? кэш? сборка?». Всё это
              время код ЗНАЛ ответ: chooseFieldBaseMap возвращает `reason`
              строкой, и она молча выбрасывалась. Прибор, знающий причину и
              не называющий её, заставляет гадать — тот же §4.0, только не о
        ...
        <div className="px-4 py-3 text-xs" style={{ color: 'var(--text-muted)', borderTop: '1px solid #21262d' }}>
          {tileDl && tileDl.total > 0 ? (
            <div className="flex items-center gap-2">
              ...
              Сохраняем карту: {tileDl.done} / {tileDl.total} {tileDl.unit}
            ...
            <div className="flex flex-wrap items-center gap-x-3 gap-y-1">
              ...
              <span>
                {mapPlan.coverage === 'packs'
                  ? `клетки карты вокруг маршрута целиком, файлов ${mapPlan.tiles}`
                    + (mapPlan.unknownSize > 0 ? ` (у ${mapPlan.unknownSize} вес неизвестен)` : '')
      ...
      {/* Ряд марок чужих навигаторов снят 13.09 — владелец: «кнопка навигация
          до сих пор открывает сторонние сервисы».
          Отменяет решение 11.08 («смысл людям пользоваться нашей кривой, если
          есть другие»), и вместе с ним — довод, который тут стоял: тогда наш
          роутер на пробе отвечал «пути нет» на четыре километра по городу.
          ЗАМЕР ТОТ НЕ ПОВТОРЯЛСЯ, и это стоит помнить: если граф откажет,
          запасного навигатора под рукой у человека больше нет. Отказ обязан
          называться словами и отдавать координаты — см. PlaceOwnRoute. */}
