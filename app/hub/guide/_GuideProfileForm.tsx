...
/**
 * Живая вкладка «Профиль» гида: загрузка из GET /api/partners/profile,
 * сохранение через PATCH /api/partners/profile (тот же контракт, что
 * онбординг-визард). Заменяет прежнюю мёртвую статичную форму дашборда.
 */
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-lg p-4 space-y-3">
        ...
        <div>
          <label htmlFor="gp-name" className={LABEL}>Имя / название</label>
          <input id="gp-name" className={INPUT} value={name} maxLength={255}
            placeholder="Иван Камчатский, гид" onChange={e => { setName(e.target.value); setSaved(false); }} />
        </div>
        <div>
          <label htmlFor="gp-desc" className={LABEL}>О себе</label>
          <textarea id="gp-desc" className={`${INPUT} resize-none`} rows={4} value={description} maxLength={2000}
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] rounded-lg p-4 space-y-3">
        ...
        <div>
          <label htmlFor="gp-phone" className={LABEL}><Phone className="w-3 h-3 inline mr-1" />Телефон</label>
          <input id="gp-phone" className={INPUT} value={phone}
            placeholder="+7 (900) 000-00-00" onChange={e => { setPhone(e.target.value); setSaved(false); }} />
        </div>
        <div>
          <label htmlFor="gp-tg" className={LABEL}>Telegram</label>
          <input id="gp-tg" className={INPUT} value={telegram}
            placeholder="@username" onChange={e => { setTelegram(e.target.value); setSaved(false); }} />
        </div>
        <div>
          <label htmlFor="gp-web" className={LABEL}><Globe className="w-3 h-3 inline mr-1" />Сайт</label>
          <input id="gp-web" className={INPUT} value={website}
