...
**Glassmorphism (решение владельца 2026-08-15, расширяет 2026-07-03):** стекло разрешено
как полноценный элемент языка Ведара — поверх фото, карт, рельефных/топографических текстур
и тёмных подложек, включая тёмные экраны полевого контура. Контракт: **«стекло — для
контекста, непрозрачность — для действия»**:
- слои контекста (шапки маршрута, статус-строки, обзорные и action-карточки, bottom nav)
  могут быть стеклом: `backdrop-blur-md bg-black/40 border border-white/15 rounded-2xl`
  (плотнее — `bg-black/60` — когда под стеклом сложный фон, а на карточке текст/CTA);
- критичные приборы и действия — SOS, offline/предупреждения, главная цифра навигации,
  compass, confirmation-диалоги — **всегда непрозрачные**, максимум тонкая стеклянная кромка;
- подложка — только чёрная/белая альфа или CSS-токены, не хардкод-hex;
- `prefers-reduced-transparency` / слабые устройства → фолбэк без blur на сплошной
  `var(--bg-card)`; смысл и layout не должны зависеть от эффекта;
- на светлых сплошных фонах (светлая тема, `--bg-primary`) стекла по-прежнему нет — там
  блюрить нечего, только сплошной `var(--bg-card)`;
- у отдельных поверхностей есть свои более строгие стандарты со сторожами (карточка тура §11:
  стекло только в герое поверх фото) — они действуют, пока владелец не изменит их отдельно.

### Компоненты платформы

- Хедер: `KH` логотип + иконка темы + ЛК (без поиска в шапке)
- Поиск: только иконка → модальное окно
- Mobile navbar: Дом / Карта / Кузьмич (по центру, медальон) / Туры / На маршруте
  (4-й пункт «Туры» ведёт на `/catalog` — витрину operator_tours; заменил «Поездки»
  по решению владельца: на телефоне коммерция была спрятана, туров не было ни в
  навигации, ни на главной. Поездки туриста доступны из ЛК в шапке.)
  (`components/shared/BottomNav.tsx`, решение владельца 2026-07-18 — единая навигация
  вместо трёх разных; состав из пяти и центр-Кузьмич — решение 2026-08-01). ЛК только в шапке.
  СОС — фиксированная кнопка в шапке на каждом экране (`components/shared/EmergencyAction.tsx`,
...
6. Glassmorphism — по контракту «стекло для контекста, непрозрачность для действия» (решение владельца 2026-08-15): поверх фото, карт, рельефных текстур и тёмных подложек; критичные действия (SOS, warnings, confirmation) — всегда непрозрачные; на светлых сплошных фонах стекла нет — см. §2
...
ПЛАНЕР lib/planner/                           — планирование поездки:
         ...
         (интересы→маршруты). Точка входа — lib/planner (barrel).
ПОИСК  lib/search/                            — поиск туров:
         tour-search (фильтр каталога) · tour-recommend (персональные).
         Точка входа — lib/search (barrel).
КУЗЬМИЧ поверхность (lib/kuzmich, чат) — зовёт движки выше, своего подбора НЕТ.
```
Отдельно: `lib/routes/catalog-query.ts` — поиск МАРШРУТОВ/МЕСТ (не туров), другой домен.
...
- 778 точек (places): все с координатами, все с описаниями ≥300 символов
...
**ВАЖНО:** перед любым импортом проверять реальное состояние через `/hub/admin/content/routes` или `/api/admin/audit-knowledge`, не полагаться на цифры выше — они могут устареть.
...
|-------|-----|------------|
| **Watchdog** | Cron 30 мин | **Единственный сторож** операционной безопасности: SOS-таймаут >15 мин (координаты + 112), брони без подтверждения >24ч, операторы игнорируют бронь >48ч, лиды >2ч, брони жилья >24ч, сейсмо-крон мёртв >15 мин, любой safety-крон из реестра мёртв (liveness по `lib/agents/cron-registry`). Алерты в Telegram. |
...
**Compliance Sentinel (152-ФЗ, детерминированный).** Весь ИИ-трафик уходит в зарубежные LLM (DeepSeek/Gemini/OpenRouter — Китай/США/ЕС), кроме одного домашнего стока YandexGPT (`llm.api.cloud.yandex.net`, РФ). Значит телефон/почта/имя туриста в тексте промпта = трансграничная передача ПД. Два CI-гарда (`lib/agents/compliance/`, гоняются в `vitest`, блокируют мерж):
- **D1 `pii-flow-scanner`** — сканит исходники, ловит интерполяцию сырых ПД (`.phone`/`.email`/персональные `*_name`/`lead.name`) в ТЕКСТ ПРОМПТА без `redactPII()`/плейсхолдера `{name}`. Чистить ПД перед LLM — `lib/security/pii-redact.ts` (`redactPII`). Осознанное исключение — запись в `ALLOWLIST` (`scan-repo.ts`).
- **D2 `provider-registry`** — замороженный реестр LLM-хостов. Новый провайдер в `lib/ai/providers.ts` → тест падает, пока хост не внесён с юрисдикцией и флагом `domestic`.

Не расширять «правилами в промпте» — только детерминированным гардом. Полноценный ИИ-«юрист» апреля 2026 удалён как театральный; Sentinel — его несценический, детерминированный преемник.
...
## 11. КАРТОЧКА ТУРА (стандарт, решение владельца 03.08)

Тур — коммерческий продукт оператора. Точка — факт, маршрут — инструкция, тур — предложение.
Карточка тура отвечает: что за поездка, как проходит день, что взять, что опасно, сколько стоит,
кто проводит и как забронировать.

**Единственная реализация — `app/marketplace/tours/[id]/_TourDetailClient.tsx`.**
`/catalog/tours/[id]` и `/marketplace/tours/[id]` рендерят её же. Заводить вторую карточку тура
запрещено — именно из-за расхождения копий карточка на проде месяцами показывала демо-оператора
и одно фото.

### Блоки (сверху вниз)

1. **Кино-герой** — фото на всю ширину, лайтбокс. Поверх фото (только здесь стекло):
   статус дня из `/api/public/safety-status` (fail-soft: нет данных — блока нет) и сезон.
2. **Заголовок** — хлебные крошки, тип активности, название (Playfair), локация, размер группы.
3. **Инструментная полоса** — сезон / длительность / группа / сложность. Только то, что есть в БД.
4. **Филмстрип** — остальные фото галереи.
5. **О туре** — `description` + чипы (сложность, сезон, погода). `short_description`
   живёт в ГЕРОЕ под заголовком (SEO-аудит 06.08: цитируемый ответ в первом
   экране — AI-поиск берёт ~44% цитат из первых 30% страницы); в «О туре» его
   не дублировать.
   **Описание не пересказывает программу и состав** — у них свои поля (`program`,
   `included`, `not_included`, `what_to_bring`). Пока полей не было, весь анонс
   оператора лежал в `description` (793); поля завели (796, 809), а описание не
   почистили — программа оказалась в трёх местах, и в описании остался старый
   оборот о сборе, исправленный только в `program` (814). Дубль сам по себе
   неопрятен, а расходящийся дубль — это две разные версии одного дня на одной
   странице. Сторож: `tests/unit/tour-content-no-duplication.test.ts`.
6. **Программа** — аккордеон из `operator_tours.program` (JSONB `[{title,text}]`).
   Перед программой, если в текстах тура упомянуты виды рыб — блок «Когда какая
   рыба клюёт?» (решение владельца 06.08): сезонная сетка из ЕДИНОГО справочника
   `lib/fish-species.ts` (тот же, что кормит `/fish` и ссылки в описаниях,
   детект — `detectFishSpecies`). Свой список видов/сезонов заводить нельзя;
   нет упоминаний — нет блока. Сторож: `tests/unit/fish-season-card.test.ts`.
7. **Что включено / не входит** — `included[]` / `not_included[]`.
8. **Что взять с собой** — `what_to_bring[]` как рабочий чек-лист (состояние в сессии).
9. **Безопасность** — `SafetyWarnings` (живые сигналы платформы) + правила тура из `safety_notes[]`.
10. **Точка сбора** — `meeting_point`.
11. **Оператор** — имя, отметка проверки, кнопка чата платформы + прямые контакты из `partners.contacts`.
12. **Отзывы** → **Кузьмич** → **липкая бронь** (цена, `BookingFormClient`). На этом карточка заканчивается.

### Источники данных

| Блок | Источник |
|------|----------|
| Фото, цена, состав | `operator_tours` |
| Программа дня | `operator_tours.program` (JSONB, миграция 809) |
| Правила безопасности | `operator_tours.safety_notes` (TEXT[], 809) |
| Живые предупреждения | `SafetyWarnings` по `tourId` |
| Статус дня | `GET /api/public/safety-status` |
| Свободные даты | `GET /api/tours/[id]/slots` (реальная занятость, через `TourDateField`) |
| Оператор и контакты | `partners.name`, `partners.contacts` |

### Запрещено

- **Своя SOS-кнопка.** SOS — только `components/shared/EmergencyAction.tsx`, всегда `--danger`,
  с офлайн-веткой. Копия расходится поведением — это уже случалось (#887).
- **Захардкоженные правила безопасности.** «Спасательные жилеты» — правда для сплава и ложь для
  вертолётной экскурсии. Только `safety_notes` конкретного тура.
- **Выдуманные поля** (`available_dates`, `contact_phones`) — даты и телефоны уже имеют источники.
- **Партнёрские блоки агрегаторов** (`RouteAffiliateBlock`, `YandexTravelBlock`) — решение владельца
  04.08. Они выводят рекламный дисклеймер с ИНН посторонних юрлиц («Go Travel Un Limited»,
  «ООО Спутник», «Яндекс Вертикали») прямо под карточкой тура НАШЕГО проверенного оператора —
  турист видит подмену продавца. Карточка тура продаёт тур оператора, а не чужие сервисы.
  Место таких блоков — страница поездки/маршрута, не карточка тура.
- Стекло на сплошных фонах (только поверх фото в герое), хардкод hex, эмодзи, `@keyframes`,
  `font-black`, дисплейный шрифт кроме Playfair.
- Показывать цену/бронь на карточке ТОЧКИ (§9) — это разные сущности.

Сторож: `tests/unit/tour-card-standard.test.ts`.

---

---

## 12. СТАНДАРТ КАРТЫ (линия называет своё происхождение)

На карте линия — это обещание. Сплошная зелёная в четыре пикселя читается как
«здесь идут», и человек по ней пойдёт. Значит вид линии обязан соответствовать
тому, откуда она взялась, а не тому, на каком экране нарисована.

**Единственный источник вида — `lib/map/line-standard.ts`.** Собирать
`color`/`weight`/`dashArray` руками в объекте геометрии запрещено: правило,
реализованное на трёх экранах трижды, — это три правила, и они уже расходились
(карточка маршрута рисовала набросок сплошной красной, планер соединял дни
плана через хребты сплошной оранжевой под словом «Маршрут»).

### Три рода линий

| Род | Что это | Вид | Функция |
|-----|---------|-----|---------|
| **Трек** | снятый путь, по нему идут | сплошная, зелёная, 4px | `trackLine()` → `surveyed` |
| **Набросок** | геометрия маршрута прямыми между точками (миграция 168, ~150 маршрутов) | пунктир, приглушённый, 2px + подпись | `trackLine()` → `sketch` |
| **Построение** | не путь вовсе: подход по азимуту, порядок дней плана, связка | пунктир, серый, 2px | `connectorLine()` |

Между наброском и построением разница не в виде, а в природе: набросок
претендует на маршрут и лжёт о его форме, построение маршрутом быть не
пытается. Поэтому у построения отдельная функция и **обязательное честное имя**
(`CONNECTOR_TITLES`) — слово решает не меньше вида.

### Правила

- Геометрию маршрута рисовать только через `trackLine()`; `null` — линии нет,
  и рисовать нечего (стиль «по умолчанию» нарисовал бы её там, где её нет).
- Набросок обязан иметь **подпись словами** рядом с картой: на карточке в 240
  пикселей пунктир от сплошной отличит не каждый глаз, а решение «идти по этой
  линии» человек принимает именно там.
- Ломаная по путевым точкам — набросок ПО ПОСТРОЕНИЮ, даже если точек много.
- Линия-построение никогда не называется «Маршрут».
- Плотность точек судит `lib/routes/track-fidelity` — своих порогов не заводить.

Сторож: `tests/unit/map-line-standard.test.ts` (ищет собранные вручную толстые
линии на всех поверхностях, где рисуется путь).
