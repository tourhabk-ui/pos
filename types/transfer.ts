...
export interface TransferOption {
  ...
  operator: {
    ...
    email: string;
    /**
     * Оценка перевозчика; `null` — оценок нет.
     *
     * Тип обязан допускать отсутствие. Пока он этого не допускал, поиск
     * подставлял 4.5 с пометкой «Заглушка»: турист видел оценку, которую
     * никто не выставлял, и выбирал по ней перевозчика. Число, обязанное
     * существовать, ПРИНУЖДАЕТ его выдумать.
     */
    rating: number | null;
  };
  stops: TransferStop[];
